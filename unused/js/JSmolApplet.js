...
// SwingJS version 1/31/2018 3:25:55 PM
...
//				break;
    ...
    Info._platform = "org.jmol.awtjs2d.Platform";///NEW
    ///Info._platform = "J.awtjs2d.Platform";///OLD
	...
	*/
    ...
    Clazz.loadClass("org.jmol.appletjs.Jmol"); /// NEW
    return Clazz.new_(org.jmol.appletjs.Jmol.c$$java_util_Map, [viewerOptions]);
///		return new J.appletjs.Jmol(viewerOptions);///OLD
	}
  ...
  proto._getAtomCorrelation = function(molData) {
    ...
    Jmol._hideLocalFileReader(this);
		this._applet.script$S(script);
	}

	proto._syncScript = function(script) {
		this._applet.syncScript$S(script);
	}

	proto._scriptCheck = function(script) {
		return this._ready && this._applet.scriptCheck$S(script);  
	}
	...
	proto._scriptWaitOutput = function(script) {
		...
		try {
			if(script) {
				ret += this._applet.scriptWaitOutput$S(script);
			}
		} catch(e) {
		}
		return ret;
	...
	proto._getPropertyAsString = function(sKey,sValue) {
		...
		return this._applet.getPropertyAsString$S$S(sKey, sValue) + "";
	}

	proto._getPropertyAsJSON = function(sKey,sValue) {
		sValue == undefined && ( sValue = "");
		try {
			return (this._applet.getPropertyAsJSON$S$S(sKey, sValue) + "");
		} catch(e) {
			return "";
		}
	}
	...
	proto._getPropertyAsJavaObject = function(sKey,sValue) {		
		sValue == undefined && ( sValue = "");
		return this._applet.getProperty$S$S(sKey,sValue);
	}
  ...
  proto._evaluate = function(expr) {  
	...
	proto.__loadModel = function(mol, script, chemID, _jmol__loadModel) {
		...
		if (!script && this._noscript) {
			this._applet.loadInlineString$S$S$Z(mol, "", false);
		} else {
  ...
  proto._loadMolData = function(mol, script, isAppend) {
		...
		this._applet.scriptWait$S(script);
  }
