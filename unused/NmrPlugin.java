...
public class NmrPlugin implements JmolPlugin {
  ...
  private void startApp(Viewer vwr) {
    
    jmolUnits = (String) jmolAppViewer.evaluateExpression("measurementUnits");
    Map<String, Object> options = vwr.vwrOptions;
    Object o = options.get("noConsole");
    nmrApp = new Nmr(new String[] { "--Plugin", "-g" + jmolAppViewer.getScreenWidth() + "x"
        + jmolAppViewer.getScreenHeight(), (o == Boolean.TRUE ? "-o" : "--") }, this);
    nmrApp.getFrame().addWindowListener(new WindowListener() {

      @Override
      public void windowOpened(WindowEvent e) {
        if (notification != null) {
          JOptionPane.showMessageDialog(nmrApp.getFrame(), notification);
          nmrApp.getCoupleTable().setCHequation("none");
        }
        notification = null;
      }
  ...
  private void transferStateFromJmol() {
    ...
    nmrApp.getViewer().script(state);
    jmolAppViewer.script("select none;set measurementUnits HZ");
    nmrApp.getPanel().updateModel(Integer.MIN_VALUE);
  }

  void transferStateToJmol() {
    jmolAppViewer.script(nmrApp.getViewer().getStateInfo());
  ...
  public void setVisible(boolean b) {
    ...
    if (started) {
      ...
      nmrApp.getFrame().setVisible(b);
    }
  }

  @Override
  public boolean isVisible() {
    return (nmrApp != null && nmrApp.getFrame().isVisible());
  }
