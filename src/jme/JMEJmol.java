...
public class JMEJmol extends JME implements WindowListener {
  ...
  private JFrame getJmolFrame(String type, boolean exit0) {
    ...
    JFrame frame = new JFrame(
        type == "search" ? "Substructure search" : getTitle());
  ...
  protected void toInChI() {
    ...
    JOptionPane.showInputDialog(this, "Standard InChI", null,
        JOptionPane.INFORMATION_MESSAGE, null, null, inchi);
  }

  /**
   * Check for smarts in this structure.
   * 
   * For this search we want the most general (c1ccccn1) as the target, which is
   * this structure. So we use JME's native code, which tends to be most
   * inclusive on aromaticity, thus super.smiles();
   * 
   * 
   * @param pattern
   * @param isSmarts
  ...
  /**
   * See if we can match this or the given smarts to any structure in an array
   * of structures.
   * 
   ...
   */
  public int[] findMatchingStructures(String smarts, String[] smilesSet,
                                      boolean isSmarts) {
    try {
      return vwr.getSmilesMatcher().hasStructure(
          smarts == null ? smilesFromJmol() : smarts, smilesSet, 0);
    } catch (Exception e) {
  ...
  public SmilesMatcher getSmilesMatcher() {
    return (smilesMatcher == null ? (smilesMatcher = new SmilesMatcher())
        : smilesMatcher);
  }
  ...
  public Dimension getImageSize() {
    Rectangle2D.Double coordBox = activeMol
        .computeBoundingBoxWithAtomLabels(null);
    double f = molecularAreaScalePixelsPerCoord;
    return new Dimension((int) (coordBox.width * f),
        (int) (coordBox.height * f));
  }

  public String toHtmlDataURI() {
    return "data:image/png;base64,"
        + javajs.util.Base64.getBase64(toBorderedPNG(null, 10, 10));
  }
  ...
  public byte[] toBorderedPNG(String filename, int marginX, int marginY) {
    try {
      ...
      toFile(headless && filename.indexOf("?") >= 0 ? "jmol.png" : filename,
          bos.toByteArray(), "png");
  ...
  public String inchi(String options) {
    return vwr.getInchi(null, molFile(),
        (options == null ? "standard" : options));
  }
  ...
  private String getMolFromSmiles(String smiles, boolean is3D) {
    ...
    String url = JC.resolveDataBase("smiles" + (is3D ? "3D" : "2D"),
        PT.escapeUrl(smiles), null);
  ...
  void sorry(String msg) {
    ...
    if (!headless)
      JOptionPane.showMessageDialog(this, msg, "Sorry, can't do that.",
  ...
  public static void createJMEFromJmolAdapter(JMECore mol,
                                              JmolAdapterAtomIterator atomIterator,
                                              JmolAdapterBondIterator bondIterator) {
    Map<Object, Integer> atomMap = new Hashtable<Object, Integer>();
    while (atomIterator.hasNext()) {
      String sym = Elements
          .elementSymbolFromNumber(atomIterator.getElementNumber());
      ...
      mol.setAtom(mol.natoms,
          JmolAdapter.getElementSymbol(atomIterator.getElement()));
    }
  ...
  protected void readDroppedData(Object newValue) {
    ...
    try {
      ...
      System.err.println("JME error reading data starting with "
          + data.substring(Math.min(data.length(), 100)));
    }
  ...
  protected void readDroppedTextFile(String fileName) {
    ...
    try {
      fis = (fileName.indexOf("://") >= 0 ? new URL(fileName).openStream()
  ...
  private void toFile(String name, final Object bytesOrString,
                      final String type) {
    boolean useThread = (name.indexOf("?") >= 0);
    if (useThread && headless) {
      sorry(
          "Filenames must not contain '?' in headless mode - '?' replaced with '_'");
      name = name.replace('?', '_');
      useThread = false;
    }
  ...
  private String getTitle() {
    return "JME-SwingJS 2D Molecular Editor"
