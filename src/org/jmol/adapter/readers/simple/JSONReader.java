...
//
//  "atomArray" : [
//    { "element" : "C" , "x":-2.03690, "y":-0.20890, "z": 0.69830 },
//    ...
//  "bondArray" : [
//   { "a1": 28, "a2": 11, "type": "SINGLE"   },
//  ...
  ...
  public void initializeReader() throws Exception {
    ...
    if (s.indexOf("atomArray:[") >= 0) {
      readAtomArray(getSection(s, "atomArray", true));
      readBondArray(getSection(s, "bondArray", true));
    } else {
  ...
  private void readAtomArray(String[] atoms) {
    for (int i = 0; i < atoms.length; ++i) {
      String[] lxyz = PT.split(atoms[i], ":");
      Atom atom = asc.addNewAtom();
      double x = 0, y = 0, z = 0;
  ...
  private void readBondArray(String[] bonds) {
    for (int i = 0; i < bonds.length; ++i) {
      String[] beo = PT.split(bonds[i], ":");
      int b = 0, e = 0, order = 1;
      for (int j = 0; j < beo.length; j += 2) {
        if (beo[j].length() > 1) {
          ...
          case '1': // a1
            b = parseIntStr(beo[j + 1]) - 1;
            break;
          case '2': //a1
            e = parseIntStr(beo[j + 1]) - 1;
            break;
          case 'y': // type
            String type = beo[j + 1];
          ...
          // ignore probably empty space
          System.err.println("JSONReader error bond tag " + beo[j]);
        }
      }
      if (b >= asc.ac || e >= asc.ac)
        System.err.println("JSONReader error bond referencing atoms " + b + " " + e + " atomCount=" + asc.ac + " for " + fileName);
      else
        asc.addBond(new Bond(b, e, order));
    }
  }
