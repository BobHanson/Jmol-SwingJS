...
public abstract class InChIC {
  ...
  public static final int INCHI_BOND_TYPE_ALTERN = 4; /* avoid by all means */
  // } INCHI_BondType;
  ...
  /* stereobond-related */
  public static final int INCHI_BOND_STEREO_DOUBLE_EITHER = 3; /* unknown stereobond geometry */
  // } INCHI_BondStereo2D;

  /*************************************************************************
   * Notes on using INCHI_BOND_STEREO_SINGLE_* from inchi_BondStereo2D * * These
   * stereo markings are used by InChI to characterize a stereogenic * atom if
   * and only if all neighbors of this atom have same z-coordinate * as this
   * atom (that is; in case of 2D fragment). * The only exception is
   * INCHI_BOND_STEREO_SINGLE_?EITHER marking which * always assigns to the atom
   * an "unknown" parity (u). * * Note the behavior which is default for InChI
   * software v. 1.03/1.02std * (at -NEWPSOFF option is not supplied) 2D stereo
   * interpretation: * only bonds that have sharp end pointing to the
   * stereogenic atom are * considered as being out of plane and only sharp ends
   * of * INCHI_BOND_STEREO_SINGLE_?EITHER bonds are considered to determine *
   * whether the stereochemistry is unknown. *
   *************************************************************************/

  /* sizes definitions */
  public final static int MAXVAL = 20; /* max number of bonds per atom                 */
  public final static int ATOM_EL_LEN = 6; /* length of ASCIIZ element symbol field        */
  public final static int NUM_H_ISOTOPES = 3; /* number of hydrogen isotopes: protium; D; T   */
  public final static int ISOTOPIC_SHIFT_FLAG = 10000; /* add to isotopic mass if isotopic_mass =      */
  /* (isotopic mass - average atomic mass)        */
  public final static int ISOTOPIC_SHIFT_MAX = 100; /* max abs(isotopic mass - average atomic mass) */

  //  #ifndef INCHI_US_CHAR_DEF
  //  // typedef signed char   S_CHAR;  implemented as byte
  //  // typedef unsigned char U_CHAR;  implemented as byte/*U*/
  ...
  // typedef struct tagInchiAtom {
      ...
      double z;
      /* connectivity */
      short  neighbor[] = new short[MAXVAL];     /* adjacency list: ordering numbers of */
                                    /*            the adjacent atoms; >= 0 */
      byte  bond_type[] = new byte[MAXVAL];    /* inchi_BondType */
      /* 2D stereo */
      byte  bond_stereo[] = new byte[MAXVAL];  /* inchi_BondStereo2D; negative if the */
                                    /* sharp end points to opposite atom */
      /* other atom properties */
      char    elname[] = new char[ATOM_EL_LEN];  /* zero-terminated chemical element name:*/
                                    /* "H"; "Si"; etc. */
      short  num_bonds;            /* number of neighbors; bond types and bond*/
                                    /* stereo in the adjacency list */
      byte  num_iso_H[] = new byte[NUM_H_ISOTOPES+1]; /* implicit hydrogen atoms */
      ...
      short  isotopic_mass;        /* 0 => non-isotopic; isotopic mass or  */
                                    /* ISOTOPIC_SHIFT_FLAG + mass - (average atomic mass) */
      byte  radical;              /* inchi_Radical */
      byte  charge;               /* positive or negative; 0 => no charge */
  
      byte[] data;
      int LEN = 8*3 + 4 * MAXVAL + ATOM_EL_LEN + 2 + NUM_H_ISOTOPES+ 1 + 2 + 1 + 1;
      byte[] getData() {
        try {
        ...
        DataOutputStream os = new DataOutputStream(bos);
          ...
          os.writeDouble(y);
          os.writeDouble(z);
          writeShorts(os, neighbor, MAXVAL);
          os.write(bond_type);
          os.write(bond_stereo);
          os.writeChars(new String(elname));
          os.write(num_bonds);
          os.write(num_iso_H);
          os.write(isotopic_mass);
          os.write(radical);
  ...
  public static void writeShorts(DataOutputStream os, short[] neighbor,
                                 int maxval) throws IOException {
    for (int i = 0; i < maxval; i++)
      os.writeShort(neighbor[i]);
  }
  ...
  ****************************************************************************/
  ...
  public final static int  NO_ATOM =          -1; /* non-existent (central) atom */

  /* 0D parity types */
  // typedef enum tagINCHIStereoType0D {
     public final static int INCHI_StereoType_None        = 0;
     public final static int INCHI_StereoType_DoubleBond  = 1;
     public final static int INCHI_StereoType_Tetrahedral = 2;
     public final static int INCHI_StereoType_Allene      = 3;
  // } INCHI_StereoType0D;

  /* 0D parities */
  // typedef enum tagINCHIStereoParity0D {
     public final static int INCHI_PARITY_NONE      = 0;
     public final static int INCHI_PARITY_ODD       = 1;  /* 'o' */
     public final static int INCHI_PARITY_EVEN      = 2;  /* 'e' */
     public final static int INCHI_PARITY_UNKNOWN   = 3;  /* 'u' */ /* (see also readinch.c)
                                            used in: Extract0DParities; INChITo_Atom  */
     public final static int INCHI_PARITY_UNDEFINED = 4;   /* '?' -- should not be used; however; see Note above */
   ...
   *************************************************/
     ...
     public class inchi_Stereo0D {

  // typedef struct tagINCHIStereo0D {
      short  neighbor[] = new short[4];    /* 4 atoms always */
      short  central_atom;   /* central tetrahedral atom or a central */
                              /* atom of allene; otherwise NO_ATOM */
      byte  type;           /* inchi_StereoType0D */
      byte  parity;         /* inchi_StereoParity0D: may be a combination of two parities: */
      ...
      byte[] getData() {
        ...
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        DataOutputStream os = new DataOutputStream(bos);
        writeShorts(os, neighbor, 4);
          os.write(central_atom);
          os.write(type);
          os.write(parity);
          return bos.toByteArray();
        } catch (IOException e) {
 ...
 public class inchi_Input {
  ...
  // typedef struct tagINCHI_Input {
      ...
      String         szOptions;    /* InChI options: space-delimited; each is preceded by */
                                    /* '/' or '-' depending on OS and compiler */
      short          num_atoms;    /* number of atoms in the structure < 1024 */
      short          num_stereo0D; /* number of 0D stereo elements */
      byte[] data;
      
  }//inchi_Input;

 public class inchi_InputINCHI {
  /* InChI -> Structure; GetStructFromINCHI()/GetStructFromStdINCHI() */
  // typedef struct tagINCHI_InputINCHI {
      /* the caller is responsible for the data allocation and deallocation */
      String szInChI;     /* InChI ASCIIZ string to be converted to a strucure */
      String szOptions;   /* InChI options: space-delimited; each is preceded by */
                         /* '/' or '-' depending on OS and compiler */
   } //INCHI_InputINCHI;
 ...
 // typedef struct tagINCHI_Output {
     ...
     String szInChI;     /* InChI ASCIIZ string */
     String szAuxInfo;   /* Aux info ASCIIZ string */
     String szMessage;   /* Error/warning ASCIIZ message */
     String szLog;       /* log-file ASCIIZ string; contains a human-readable list */
                        /* of recognized options and possibly an Error/warning message */
  } // INCHI_Output;

 

 public class inchi_OutputStruct {
 ...
 // typedef struct tagINCHI_OutputStruct {
     ...
     String           szLog;        /* log-file ASCIIZ string; contains a human-readable list */
                                   /* of recognized options and possibly an Error/warning message */
     long[][]  WarningFlags = new long[2][2]; /* warnings; see INCHIDIFF in inchicmp.h */
 ...
 */
 // typedef enum tagRetValGetINCHI {
     public static final int inchi_Ret_SKIP    = -2; /* not used in InChI library */
     public static final int inchi_Ret_EOF     = -1; /* no structural data has been provided */
     public static final int inchi_Ret_OKAY    =  0; /* Success; no errors or warnings */
     public static final int inchi_Ret_WARNING =  1; /* Success; warning(s) issued */
     public static final int inchi_Ret_ERROR   =  2; /* Error: no InChI has been created */
     public static final int inchi_Ret_FATAL   =  3; /* Severe error: no InChI has been created (typically; memory allocation failure) */
     public static final int inchi_Ret_UNKNOWN =  4; /* Unknown program error */
     public static final int inchi_Ret_BUSY    =  5;  /* Previuos call to InChI has not returned yet */
...
// {
   ...
   public static final int   INCHI_INVALID_PREFIX     =   1;
   public static final int  INCHI_INVALID_VERSION    =   2;
   public static final int INCHI_INVALID_LAYOUT     =   3;
   public static final int INCHI_FAIL_I2I           =   4;
...
//
 ...
 /*^^^ InChI PREFIX */
 public final static String INCHI_STRING_PREFIX = "InChI=";
 public final static int LEN_INCHI_STRING_PREFIX = 6;
 ...
 GetINCHI / GetStdINCHI
  ...
  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
  abstract int GetINCHI( inchi_Input inp, inchi_Output out);


 abstract int GetStdINCHI( inchi_Input inp, inchi_Output out);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
public abstract void FreeINCHI ( inchi_Output out );
public abstract void FreeStdINCHI ( inchi_Output out );
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int GetStringLength( String p );
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int GetStructFromINCHI( inchi_InputINCHI inpInChI, inchi_OutputStruct outStruct );
 public abstract int GetStructFromStdINCHI( inchi_InputINCHI inpInChI, inchi_OutputStruct outStruct );
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
public abstract void FreeStructFromINCHI( inchi_OutputStruct out );
public abstract void FreeStructFromStdINCHI( inchi_OutputStruct out );
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int GetINCHIfromINCHI( inchi_InputINCHI inpInChI, inchi_Output out );
...
// #endif
 ...
 public final static int STR_ERR_LEN   =  256;
 
 public class InchiInpData {
 // typedef struct tagInchiInpData {
     ...
     char         szErrMsg[] = new char[STR_ERR_LEN];
 } //InchiInpData;
...
// #endif
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int Get_inchi_Input_FromAuxInfo( 
                                                         String szInchiAuxInfo, 
                                                         int bDoNotAddH, 
                                                         int bDiffUnkUndfStereo,
                                                         InchiInpData pInchiInp );
 public abstract int Get_std_inchi_Input_FromAuxInfo( String szInchiAuxInfo, 
                                                         int bDoNotAddH, 
                                                         InchiInpData pInchiInp );
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
public abstract void Free_inchi_Input( inchi_Input pInp );
public abstract void Free_std_inchi_Input( inchi_Input pInp );
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int CheckINCHI(String szINCHI, int strict);    
...
// #endif
 ...
 public final static int INCHIKEY_OK = 0;
 public final static int INCHIKEY_UNKNOWN_ERROR = 1;
 public final static int INCHIKEY_EMPTY_INPUT = 2;
 public final static int INCHIKEY_INVALID_INCHI_PREFIX = 3;
 public final static int INCHIKEY_NOT_ENOUGH_MEMORY = 4;
 public final static int INCHIKEY_INVALID_INCHI = 20;
 public final static int INCHIKEY_INVALID_STD_INCHI = 21;


 /*^^^ Return codes for CheckINCHIKey */
 // typedef enum tagRetValGetINCHIKey
// {
   public final static int      INCHIKEY_VALID_STANDARD     =   0;
     public final static int     INCHIKEY_VALID_NON_STANDARD =  -1;
     public final static int  INCHIKEY_INVALID_LENGTH     =   1;
     public final static int  INCHIKEY_INVALID_LAYOUT     =   2;
     public final static int    INCHIKEY_INVALID_VERSION    =   3;
// } RetValCheckINCHIKeyv;
 ...
 /* To compile all InChI code as a C++ code public final static int INCHI_ALL_CPP */
...
// #endif
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int GetINCHIKeyFromINCHI(String szINCHISource,
                                                               int xtra1,
                                                               int xtra2,
                                                               String szINCHIKey, 
                                                               String szXtra1, 
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int GetStdINCHIKeyFromStdINCHI(String szINCHISource, 
                                                                     String szINCHIKey);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int CheckINCHIKey(String szINCHIKey);    
...
// #endif
 ...
 public static final int INCHI_NUM          =  2;   /* = array size; member indexes: */
 ...
 public class NORM_ATOM
 // typedef struct tagNormAtom 
 {
     char          elname[] = new char[ATOM_EL_LEN];      /* chem. element name */
     byte/*U*/        el_number;                /* number of the element in the Periodic Table */
     short      neighbor[] = new short[MAXVAL];         /* positions (from 0) of the neighbors in the NORM_ATOM array */
     short      orig_at_number;           /* original atom number; starts from 1 */
     short      orig_compt_at_numb;       /* atom number within a component before terminal H removal */
     byte        bond_stereo[] = new byte[MAXVAL];      /* 1=Up;4=Either;6=Down (this atom is at the pointing wedge)
                                                negative => on the opposite side of the wedge; 3=Either double bond  */
     byte/*U*/        bond_type[] = new byte[MAXVAL];        /* 1=single; 2=double; 3=triple; 4=1/2 (bond order is 1 or 2) */
                                             /* 5=1/2/3; 6=1/3; 7=2/3; 8=tautomeric; 9=1/2 non-stereogenic */

     byte        valence;                  /* number of bonds = number of neighbors not greater than MAXVAL */
     byte        chem_bonds_valence;       /* sum of bond types (1;2;3); type 4 needs special treatment */
     byte        num_H;                    /* number of adjacent implicit hydrogen atoms including D and T    */
     byte        num_iso_H[] = new byte[NUM_H_ISOTOPES];/* number of adjacent implicit 1H(protium); 2H(D); 3H(T) < 16 */
     byte        iso_atw_diff;             /* =0 => natural isotopic abundances  */
                                             /* >0 => (isotopic mass) - (rounded average atomic mass) + 1 */
                                             /* <0 => (isotopic mass) - (rounded average atomic mass) */
     byte        charge;                   /* charge */
     byte        radical;                  /* RADICAL_SINGLET; RADICAL_DOUBLET; or RADICAL_TRIPLET */
     byte        bAmbiguousStereo;         /* flag of detected stereo ambiguity */
     byte        cFlags;                   /* AT_FLAG_ISO_H_POINT: atom may have exchangeable isotopic H */
     short      at_type;                  /* ATT_NONE; ATT_ACIDIC; etc. See InChI normalization code */
     short      component;                /* number of the structure component > 0 */
     short      endpoint;                 /* id of a tautomeric group */
     short      c_point;                  /* id of a positive charge group */
     double        x;                        /* x coordinate */
     double        y;                        /* y coordinate */
     double        z;                        /* x coordinate */
     /*---------  0D parities ----------*/
     byte        bUsed0DParity;            /* bit=1 => stereobond; bit=2 => stereocenter */
     /*-----  tetrahedral stereo parity */
     byte        p_parity;                 /* tetrahedral (sp3) cml parity */
     short      p_orig_at_num[] = new short[MAX_NUM_STEREO_ATOM_NEIGH]; /* orig_at_number of each neighbor > 0; 0=> no neighbor */
     /*----- stereo bond (SB) parities */
     byte        sb_ord[] = new byte[MAX_NUM_STEREO_BONDS];  /* neighbor[] index of another end of this SB; starts from 0 */
     byte        sn_ord[] = new byte[MAX_NUM_STEREO_BONDS];  /* neighbor[] index of a bond that is not this SB; starts from 0;
     ...
     byte        sb_parity[] = new byte[MAX_NUM_STEREO_BONDS];       /* parities of stereobonds (sp2) incident to this atom */  
     short      sn_orig_at_num[] = new short[MAX_NUM_STEREO_BONDS];  /* orig_at_number of sn_ord[] neighbor > 0 */

// #if( FIND_RING_SYSTEMS == 1 )
     byte  bCutVertex;                    /* is the atom a cut-vertex or not */
     short nRingSystem;                  /* starts from 1; number of a ring system */
     short nNumAtInRingSystem;           /* number of atoms in a ring system to which this at belongs */
     short nBlockSystem;                 /* ambiguous if the atom is a cut-vertex: better apply this to bonds */

// #if( FIND_RINS_SYSTEMS_DISTANCES == 1 )
     short nDistanceFromTerminal;        /* not used */
// #endif
//
// #endif
 }// NORM_ATOM;
 ...
 // typedef struct tagNormAtomData
 {
     ...
     short    nNumRemovedProtonsIsotopic[] = new short[NUM_H_ISOTOPES]; 
                                     /* isotopic composition of removed protons; not included in num_iso_H[] */
     short    num_iso_H[] = new short[NUM_H_ISOTOPES]; 
                                     /* isotopic H on tautomeric atoms and those 
                                        in nIsotopicEndpointAtomNumber */
     long bTautFlags;         /* for internal use */
     long bTautFlagsDone;     /* for internal use */
     long bNormalizationFlags;/* for internal use */
 } //NORM_ATOMS;
 ...
 class INCHIGEN_DATA {
     ...
     char          pStrErrStruct[] = new char[STR_ERR_LEN]; /* intermediate log (warning/error report) */
     int           num_components[] = new int[INCHI_NUM];  /* number of allocated INChI; INChI_Aux data structures */
     ...
     NORM_ATOMS   NormAtomsNontaut[] = new NORM_ATOMS[INCHI_NUM]; 
     NORM_ATOMS   NormAtomsTaut[] = new NORM_ATOMS[INCHI_NUM];
...
// #endif
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract INCHIGEN_DATA INCHIGEN_Create();
 public abstract INCHIGEN_DATA STDINCHIGEN_Create();
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int INCHIGEN_Setup(INCHIGEN_HANDLE HGen, 
                                                         INCHIGEN_DATA  pGenData, 
                                                         inchi_Input  pInp);
 public abstract int STDINCHIGEN_Setup(INCHIGEN_HANDLE HGen,
                                                            INCHIGEN_DATA pGenData, 
                                                            inchi_Input pInp);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int INCHIGEN_DoNormalization(INCHIGEN_HANDLE HGen, 
                                                                      INCHIGEN_DATA pGenData);
 public abstract int STDINCHIGEN_DoNormalization(INCHIGEN_HANDLE HGen, 
                                                                      INCHIGEN_DATA pGenData);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int INCHIGEN_DoCanonicalization
                                 (INCHIGEN_HANDLE HGen, INCHIGEN_DATA pGenData);
 public abstract int STDINCHIGEN_DoCanonicalization
                                 (INCHIGEN_HANDLE HGen, INCHIGEN_DATA pGenData);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
 public abstract int INCHIGEN_DoSerialization(INCHIGEN_HANDLE HGen,
                                                                   INCHIGEN_DATA pGenData, 
                                                                   inchi_Output pResults);
 public abstract int STDINCHIGEN_DoSerialization(INCHIGEN_HANDLE HGen, 
                                                                      INCHIGEN_DATA pGenData, 
                                                                      inchi_Output pResults);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
// public abstract void INCHI_DECL INCHIGEN_Reset(INCHIGEN_HANDLE HGen, 
//                                INCHIGEN_DATA pGenData, 
//                                inchi_Output pResults);
// public abstract void  INCHI_DECL STDINCHIGEN_Reset(INCHIGEN_HANDLE HGen, 
//                                INCHIGEN_DATA pGenData, 
//                                inchi_Output pResults);
 ...
 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
public abstract void INCHIGEN_Destroy(INCHIGEN_HANDLE HGen);
public abstract void STDINCHIGEN_Destroy(INCHIGEN_HANDLE HGen);
