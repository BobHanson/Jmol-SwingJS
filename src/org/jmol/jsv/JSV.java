...
public class JSV implements JmolJSpecView {

  private Viewer vwr;
  @Override
  public JSV setViewer(Viewer vwr) {
    this.vwr = vwr;
    return this;
  }
  ...
  private void sendJSpecView(String peak) {
    ...
    peak = vwr.fullName + JC.JSV_SYNC_KEYWORD_PREFIX + " " + peak;
  ...
  public void setModel(int modelIndex) {
    Boolean startJSV = (Boolean) vwr.ms.getInfoM(JC.INFO_START_JSPECVIEW); 
    int syncMode = (startJSV == Boolean.TRUE ? StatusManager.SYNC_DRIVER
        : vwr.sm.getSyncMode());
    if (syncMode != StatusManager.SYNC_DRIVER)
      return;
    String peak = (String) vwr.ms.getInfo(modelIndex, JC.INFO_JDX_MODEL_SELECT);
  ...
  public String processSync(String script, int jsvMode) {
    ...
    case JC.JSV_SEND_C13SIMULATE:
      if (!script.startsWith(JC.JSV_SYNC_KEYWORD_PREFIX))
        script = JC.JSV_SYNC_KEYWORD_PREFIX + script;
      script = vwr.fullName + script;
      vwr.sm.syncSend(script, ">", 0);
      return null;
    }
    switch (jsvMode) {
    default:
    ...
    case JC.JSV_SELECT:
      ...
      String id = (!isSimulation || vwr.isApplet ? ""
    ...
    case JC.JSV_SETPEAKS:
      ...
      // must be from the MOST RECENT spectrum
      ...
      vwr.ms.setInfo(vwr.am.cmi, JC.INFO_JDX_ATOM_SELECT + "_" + type, peaks);
      return null;
    }
  }
