...
public class _TachyonExporter extends __RayTracerExporter {
  ...
  private void getLightingInfo() {
    lighting = " AMBIENT " + round(gdata.getAmbientPercent() / 100d)
        + " DIFFUSE " + round(gdata.getDiffusePercent()/100d) 
        + " SPECULAR " + round(gdata.getSpecularPercent() / 100d);
...
//    output("  projection ORTHOGRAPHIC\n");
  ...
  protected void fillConicalCylinder(P3d screenA, P3d screenB,
    ...
    int diameter = (int) vwr.tm.scaleToScreen((int) ((screenA.z + screenB.z)/2d), madBond);
  ...
  protected void outputEllipsoid(P3d center, double radius, double[] coef, short colix) {
    tm.transformPt3fSafe(center, tempP1);
  ...
  protected void outputSurface(T3d[] vertices, T3d[] normals,
    ...
    if (polygonColixes != null) {
      ...
      for (int i = i0; i >= 0; i = (isAll ? i - 1 : bsPolygons.nextSetBit(i + 1))) {
        ...
        tm.transformPt3fSafe(tempP2, tempP2);
        tm.transformPt3fSafe(tempP3, tempP3);
