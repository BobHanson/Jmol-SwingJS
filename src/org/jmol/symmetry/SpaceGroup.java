...
*/ 
  ...
  private void generateMatrixOperators(HallInfo h) {
    if (h == null) {
      if (operationCount > 0)
        return;
      if (hallSymbol == null || hallSymbol.endsWith("?")) {
        checkHallOperators();
        return;
      }
      symmetryOperations = new SymmetryOperation[4];
  ...
  private final static int determineSpaceGroupIndex(String name, double a,
    ...
    if (lastIndex < 0)
      lastIndex = SG.length;
    if (getExplicitSpecialGroupType(name) > TYPE_SPACE)
      return -1;
     ...
     if (haveExtension) { // P2/m:a      
       ...
       for (i = 0; i < lastIndex; i++)
         if ((s = SG[i]).hmSymbolAbbr.equalsIgnoreCase(abbr)
             && (s.hmSymbolExt.equalsIgnoreCase(ext)
  ...
  private volatile static String lastHallSymbol = "";
  ...
  private void buildSelf(String sgLineData) {
    ...
    boolean isAlternate = jmolId.endsWith("*");
    if (isAlternate)
      jmolId = jmolId.substring(0, jmolId.length() - 1);
    ...
    if ("xyz".equals(terms[6])) {
      ...
      if (isAlternate) {
        hallSymbolAlt = hallSymbol;
        hallSymbol = lastHallSymbol;
      }
  ...
  void setHMSymbol(String name) {
    ...
    hmSymbolFull = (groupType == TYPE_SPACE ? toCap(name, 1) : name.toLowerCase());
    latticeType = Character.toUpperCase(hmSymbolFull.charAt(0));
    ...
    return (strName == null 
        ? (strName = 
          (jmolId == null || jmolId.equals("0")
            ? name 
            : jmolId + " HM:" + hmSymbolFull 
            + (getClegId() == null ? "" : " #" + getClegId())
            )
          ) 
        : strName
        );
  ...
  void checkHallOperators() {
    if (nHallOperators != null && nHallOperators.intValue() != operationCount) {
      if (hallInfo == null || hallInfo.isGenerated()) {
        ...
        transformSpaceGroup(TYPE_SPACE, this, getSpaceGroupFromJmolClegOrITA(null, itaNumber), null, itaTransform, null);
        hallInfo = null;
      }
    }
  }
  ...
  private static int addCentering(M4d trmInv, double[][] c, P3d pc, P3d p,
                                   Lst<Object> cent, int n) {
   ...
   */
  @SuppressWarnings("unchecked")
  static Map<String, Object> fillMoreData(Viewer vwr, Map<String, Object> map, String clegId,
                                                       int itno, Map<String, Object> its0) {
   ...
   */
  static SpaceGroup transformSpaceGroup(int groupType, SpaceGroup sg, SpaceGroup base,
    ...
    if (sg == null) {
      return createITASpaceGroup(groupType, xyzList, base);
    }
...
230       I a -3 d       Ia-3d     Ia-3d     -I 4bd 2c 3
  ...
  // * indicates a nonstandard Hall name  "p 21" same as "p 2yb" (removed in 16.2.28)
  //   (this was some idea that Sid Hall gave me by email, but it no
  //    longer seems to matter, and it caused trouble with modelkit assign spacegroup
  // 4:c*;2;c2^2;p 1 1 21*;p 21         
  // added # of operators to help in search phase
  // "4:b;2;c2^2;p 1 21 1;p 2yb",   //full name

//610 settings for 230 space groups (608 from ITA; 2 for added 152 and 154 c-shifts
//including also several duplicates
  //
  ...
  private final static String[] moreSettings = {
      // these are allowed only in their full-name form, with a "'" following
      "A 1 1 2 :2'","5:-a-c,a,-b",
      "A 1 2 1 :1'","5:c,-b,a",
      "B 1 1 2 :1'","5:a,c,-b",
      "B 2 1 1 :2'","5:-b,-a-c,a",
  ...
  public static boolean isInRange(double itno, int groupType, boolean allowSetIndex, boolean allow300) {
    int max = getMax(groupType);
    switch (groupType) {
    case TYPE_SPACE:
      if (allow300) {
        int i = (int) itno;
        int type = getImplicitSpecialGroupType(i);
        if (type == TYPE_PLANE)
          allowSetIndex = false;
        return (type != TYPE_INVALID && (allowSetIndex || i == itno));
      }
      break;
    case TYPE_PLANE:
      allowSetIndex = false;
  ...
  public static int getImplicitSpecialGroupType(int itno) {
    ...
    if (itno <= 230)
      return TYPE_SPACE;
    if (itno > TYPE_PLANE && itno <= TYPE_PLANE + 17) 
      return TYPE_PLANE;
    if (itno > TYPE_LAYER && itno <= TYPE_LAYER + 80) 
      return TYPE_LAYER;
    if (itno > TYPE_ROD && itno <= TYPE_ROD + 75) 
      return TYPE_ROD;
    if (itno > TYPE_FRIEZE && itno <= TYPE_FRIEZE + 7) 
      return TYPE_FRIEZE;
    return TYPE_INVALID;
  }
  ...
  public static boolean hmMatches(String hm, String name, int specialType) {
    ...
    if (specialType == TYPE_PLANE) {
      ...
      case 3:
        //        p2mm p2mg p2gg c2mm 
        // accepts pmm  pmg  pgg  cmm
        //        p4mm p4gm p6mm
        // accepts p4m  p4g  p6m
        char c2 = hm.charAt(1);
        return (c2 == '2' ?
        hm.charAt(2) == c && hm.charAt(3) == name.charAt(2)
        : hm.charAt(n - 1) == 'm' && hm.startsWith(name));
      }
    }
    return false;
  }
   ...
   * @param newParams
   * @param allowSame
   *        true to allow same-distance a,b,c for lower-symmetry sg
  ...
  public boolean createCompatibleUnitCell(double[] params, double[] newParams,
    ...
    toRhom = (n != 0 && axisChoice == 'r');
    isRhom = (toRhom && SimpleUnitCell.isRhombohedral(params));
...
//  public boolean isMonoclinicSG(int n) {
//    return (n >= 3 && n <= 15);
//  }
//  
//  public boolean isTetragonalSG(int n) {
//    return (n >= 75 && n <= 142);
//  }
//  
