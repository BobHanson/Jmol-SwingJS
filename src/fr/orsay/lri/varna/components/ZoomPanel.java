...
public class ZoomPanel extends JPanel
  ...
  /**
   * for chaining dynamic loading Interface.getInterface(....).setVP()
   * 
   * @param vp
   * @return this
   */
  ...
  public synchronized void drawPanel() {
    if (getWidth() > 0 && getHeight() > 0) {
      _bi = new BufferedImage(getWidth(), getHeight(),
          BufferedImage.TYPE_4BYTE_ABGR);
      Graphics2D g2 = _bi.createGraphics();
      g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
          RenderingHints.VALUE_ANTIALIAS_ON);
      VueVARNAGraphics g2D = new SwingGraphics(g2, false);
      rnaRect = _vp.renderRNA(g2D,
          new Rectangle2D.Double(0, 0, getWidth(), getHeight()), false, true);

      Point2D.Double p1 = _vp.panelToLogicPoint(new Point2D.Double(0., 0.));
      Point2D.Double p2 = _vp.panelToLogicPoint(
          new Point2D.Double(_vp.getWidth(), _vp.getHeight()));
      ...
      Rectangle2D.Double rvisible = new Rectangle2D.Double(
          rnaRect.x + rnaRect.width * (p1.x - rnaBox.x) / rnaBox.width,
          rnaRect.y + rnaRect.height * (p1.y - rnaBox.y) / rnaBox.height,
          ratiox * rnaRect.width, ratioy * rnaRect.height);
      ...
      g2.fillRect(0, (int) rvisible.y, (int) rvisible.x,
          (int) rvisible.height + 1);
      g2.fillRect((int) (rvisible.x + rvisible.width), (int) rvisible.y,
          (int) (getHeight() - (rvisible.x + rvisible.width)),
          (int) (rvisible.height + 1));
      g2.fillRect(0, (int) (rvisible.y + rvisible.height), getWidth(),
          (int) (getHeight() - (rvisible.y + rvisible.height)));

      g2.setColor(new Color(.7f, .7f, .7f, .3f));
      g2.draw(rvisible);
      g2.drawLine(0, 0, (int) rvisible.x, (int) rvisible.y);
      g2D.drawLine(getWidth(), 0, rvisible.x + rvisible.width, rvisible.y);
      g2D.drawLine(getWidth(), getHeight(), rvisible.x + rvisible.width,
