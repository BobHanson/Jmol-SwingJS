...
public class BaseSet {
  ...
  public BaseSet(BaseSet b) {
    _caption = b._caption;
    _bases = new HashSet<ModeleBase>(b._bases);
  }

  public BaseSet(String caption) {
    _caption = caption;
  }

  public BaseSet(String caption, ModeleBase mb) {
    this(caption);
    addBase(mb);
  }
  ...
  public ArrayList<ModeleBase> getBaseList() {
  ...
  /**
   * Returns, in a new BaseSet, the intersection of the current BaseSet and of
   * the argument.
   * 
   * @param mb
   *        The base list to be used for the intersection
   * @return The intersection of the current base list and the argument.
   */

  public BaseSet retainAll(BaseSet mb) {
    HashSet<ModeleBase> cp = new HashSet<ModeleBase>();
    cp.addAll(_bases);
    cp.retainAll(mb._bases);
    BaseSet result = new BaseSet("TmpIntersection");
    result.addBases(cp);
    return result;
  }

  /**
   * Returns, in a new BaseSet, the list consisting of the current BaseSet minus
   * the list passed as argument.
   * 
   * @param mb
   *        The base list to be subtracted from the current one
   * @return The current base list minus the list passed as argument.
   */

  public BaseSet removeAll(BaseSet mb) {
    HashSet<ModeleBase> cp = new HashSet<ModeleBase>();
    cp.addAll(_bases);
    cp.removeAll(mb._bases);
    BaseSet result = new BaseSet("TmpMinus");
    result.addBases(cp);
    return result;
  }
