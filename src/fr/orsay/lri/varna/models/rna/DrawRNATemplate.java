...
public class DrawRNATemplate {
  ...
  /**
   * Draw this RNA like the given template. The helixLengthAdjustmentMethod
   * argument tells what to do in case some helices are of a different length in
   * the template and the actual helix. See class DrawRNATemplateMethod above
   * for possible values.
   * 
   * @param template
   * @param conf
   * @param helixLengthAdjustmentMethod
   * @param curveMethod
   * @param straightBulges
   * @throws RNATemplateDrawingAlgorithmException
   */
  public void drawRNATemplate(RNATemplate template, VARNAConfig conf,
    ...
    if (helixLengthAdjustmentMethod == DrawRNATemplateMethod.MAXSCALINGFACTOR) {
      ...
      while (iter.hasNext()) {
        ...
        if (element instanceof RNATemplateHelix
          ...
          int[] basesInHelixArray = RNATemplateAlign
              .intArrayFromList(mapping.getAncestor(helix));
          Arrays.sort(basesInHelixArray);
          double l = computeLengthIncreaseFactor(basesInHelixArray, helix,
              straightBulges);
          lengthIncreaseFactor.put(helix, Double.valueOf(l));
      ...
      try {
        ...
        throw (new RNATemplateDrawingAlgorithmException(
    ...
    while (computeCoords) {
      ...
      while (iter.hasNext()) {
        ...
        if (element instanceof RNATemplateHelix
          ...
          if (!alreadyDrawnHelixes.contains(helix)) {
            ...
            drawHelixLikeTemplateHelix(basesInHelixArray, helix, coords,
                centers, angles, globalIncreaseFactor, translateVectors,
                straightBulges);
          ...
          if (lastMappedHelix != null) {
            ...
            if (lastMappedHelix == helix) {
              ...
              computeHelixEndPointDirections(howWeGotOutOfLastHelix,
                  helixVector, new Point2D.Double());
              ...
              Point2D.Double loopCenter = new Point2D.Double((P0.x + P3.x) / 2,
                  (P0.y + P3.y) / 2);
              rna.drawLoop(b1, b2, loopCenter.x, loopCenter.y, angle, coords,
              ...
              int b2 = firstTimeWeMeetThisHelix ? basesInHelixArray[0]
                  : basesInHelixArray[basesInHelixArray.length / 2];
              P0.setLocation(coords[b1]);
              P3.setLocation(coords[b2]);

              Point2D.Double P1, P2;

              if (howWeGotOutOfLastHelix
                  .getOtherElement() instanceof RNATemplateUnpairedSequence
                  && howWeGetInCurrentHelix
                      .getOtherElement() instanceof RNATemplateUnpairedSequence) {
                // We will draw the bases on a Bezier curve
                P1 = new Point2D.Double();
                computeBezierTangentVectorTarget(howWeGotOutOfLastHelix, P0,
                    P1);

                P2 = new Point2D.Double();
                computeBezierTangentVectorTarget(howWeGetInCurrentHelix, P3,
                    P2);
              } else {
              ...
              drawAlongCurve(b1, b2, P0, P1, P2, P3, coords, centers, angles,
            ...
            RNATemplateElement templateSequenceCandidate = mapping
                .getPartner(0);
            ...
            if (templateSequence != null) {
              ...
              if (howWeGetInCurrentHelix != null) {
                computeHelixEndPointDirections(howWeGetInCurrentHelix,
                    new Point2D.Double(), j);
              } else {
                j.setLocation(1, 0);
              }
              coords[b1].setLocation(coords[b2].x + j.x * idealLength,
                  coords[b2].y + j.y * idealLength);
            }
            P0.setLocation(coords[0]);

            Point2D.Double P1, P2;

            if (howWeGetInCurrentHelix
                .getOtherElement() instanceof RNATemplateUnpairedSequence
                && templateSequence != null) {
              // We will draw the bases on a Bezier curve
              P1 = new Point2D.Double();
              computeBezierTangentVectorTarget(templateSequence.getIn(), P0,
                  P1);
          ...
          if (firstTimeWeMeetThisHelix) {
            howWeGotOutOfLastHelixBaseIndex = basesInHelixArray[basesInHelixArray.length
                / 2 - 1];
          } else {
            howWeGotOutOfLastHelixBaseIndex = basesInHelixArray[basesInHelixArray.length
                - 1];
      ...
      if (howWeGotOutOfLastHelixBaseIndex < coords.length - 1 && element != null
          && coords.length > 1) {
        ...
        if (endTemplateSequence != null) {
          ...
          coords[b2].setLocation(coords[b1].x + j.x * idealLength,
              coords[b1].y + j.y * idealLength);
        }
        ...
        if (howWeGotOutOfLastHelix != null
          ...
          P2 = new Point2D.Double();
          computeBezierTangentVectorTarget(endTemplateSequence.getOut(), P3,
              P2);
        } else if (lastMappedHelix == null && beginTemplateSequence != null
            && endTemplateSequence != null) {
          // We will draw the bases on a Bezier curve
          P1 = new Point2D.Double();
          computeBezierTangentVectorTarget(beginTemplateSequence.getIn(), P0,
              P1);
        ...
        drawAlongCurve(b1, b2, P0, P1, P2, P3, coords, centers, angles,
            curveMethod,
            lastMappedHelix != null ? lastMappedHelix.isFlipped() : false,
            straightBulges);

      }

      if (helixLengthAdjustmentMethod == DrawRNATemplateMethod.NOINTERSECT
          && coords.length > 3) {
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      _listeBases.get(i)
          .setCoords(new Point2D.Double(coords[i].x * conf._spaceBetweenBases,
              coords[i].y * conf._spaceBetweenBases));
      _listeBases.get(i)
          .setCenter(new Point2D.Double(centers[i].x * conf._spaceBetweenBases,
  ...
  /**
   * IN: Argument helixEndPoint is an IN argument (will be read), and must
   * contain an helix edge endpoint.
   * 
   * The other arguments are OUT arguments (must be existing objects, content
   * will be overwritten).
   * 
   * OUT: The i argument will contain a vector colinear to the vector from the
   * helix startPosition to endPosition or the opposite depending on there the
   * endpoint is (the endpoint will be on the destination side of the vector).
   * ||i|| = 1
   * 
   * OUT: The j vector will contain an vector that is colinear to the last/first
   * base pair connection on the side of this endpoint. The vector will be
   * oriented to the side of the given endpoint. ||j|| = 1
   * @param helixEndPoint 
   * @param i 
   * @param j 
   */
  private void computeHelixEndPointDirections(EdgeEndPoint helixEndPoint, // IN
  ...
  /**
   * A cubic Bezier curve can be defined by 4 points, see
   * http://en.wikipedia.org/wiki/Bezier_curve#Cubic_B.C3.A9zier_curves For each
   * of the curve end points, there is the last/first point of the curve and a
   * point which gives the direction and length of the tangent vector on that
   * side. This two points are respectively curveEndPoint and
   * curveVectorOtherPoint. IN: Argument helixVector is the vector formed by the
   * helix, in the right direction for our sequence. IN: Argument curveEndPoint
   * is the position of the endpoint on the helix. OUT: Argument
   * curveVectorOtherPoint must be allocated and the values will be modified.
   * @param endPoint 
   * @param curveEndPoint 
   * @param curveVectorOtherPoint 
   * @throws RNATemplateDrawingAlgorithmException 
   */
  private void computeBezierTangentVectorTarget(EdgeEndPoint endPoint,
    ...
    if (endPoint.getElement() instanceof RNATemplateHelix) {
      ...
      EdgeEndPoint endPointOnHelix = sequenceEndPointIsIn
          ? sequence.getIn().getOtherEndPoint()
          : sequence.getOut().getOtherEndPoint();
      if (endPointOnHelix == null) {
        throw (new RNATemplateDrawingAlgorithmException(
            "Sequence is not connected to an helix."));
      }
    } else {
    ...
    double l = sequenceEndPointIsIn ? sequence.getInTangentVectorLength()
        : sequence.getOutTangentVectorLength();

    // Compute the absolute angle our line makes to the helix
    double theta = sequenceEndPointIsIn ? sequence.getInTangentVectorAngle()
   ...
   * @param basesInHelixArray 
   * @param helix 
   * 
   * @param straightBulges
   * @return double 
   */
  private double computeLengthIncreaseFactor(int[] basesInHelixArray, // IN
   ...
   * @return Double value
   */
  private Point2D.Double computeLengthIncreaseDelta(int[] basesInHelixArray, // IN
                                                    RNATemplateHelix helix, // IN
                                                    boolean straightBulges) {
    double templateLength = computeHelixTemplateLength(helix);
    double realLength = computeHelixRealLength(basesInHelixArray,
    ...
    return new Point2D.Double(i.x * (realLength - templateLength),
        i.y * (realLength - templateLength));
  }
   ...
   * 
   * @param helix
   *        The template helix you want to compute the vectors from.
   * @param o
   *        This point coordinates will be set the origin of the helix (or not
   *        if null), ie. the point in the middle of the base pair with the two
   *        most extreme bases.
   * @param i
   *        Will be set to the normalized helix vector. (nothing done if null)
   * @param j
   *        Will be set to the normalized helix base pair vector (5' -> 3').
   *        (nothing done if null)
   */
  private void computeTemplateHelixVectors(RNATemplateHelix helix, // IN
  ...
  private double computeHelixRealLength(int[] basesInHelixArray,
                                        boolean straightBulges) {
    return drawHelixLikeTemplateHelix(basesInHelixArray, null, null, null, null,
        0, null, straightBulges);
  }

  /**
   * Result type of countUnpairedLine(), see below.
   */
  protected static class UnpairedLineCounts {
    public int nBP, nLD, total;
  }

  /**
   * If we are drawing an unpaired region that may contains helices, and we are
   * drawing it on a line (curve or straight, doesn't matter), how many
   * intervals should have a base-pair length (start of an helix) and how many
   * should have a consecutive unpaired bases length? Returns an array with
   * three elements: - answer to first question - answer to second question -
   * sum of both, ie. total number of intervals (this is NOT lastBase-firstBase
   * because bases deep in helices do not count)
   * @param firstBase 
   * @param lastBase 
   * @return count 
   */
  ...
  /**
   * Draw the given helix (given as a *SORTED* array of indexes) like defined in
   * the given template helix. OUT: The bases positions are not changed in fact,
   * instead the coords and centers arrays are modified. IN: The helix origin
   * position is multiplied by scaleHelixOrigin and translateVectors.get(helix)
   * is added.
   * @param basesInHelixArray 
   * @param helix 
   * @param coords 
   * @param centers 
   * @param angles 
   * @param scaleHelixOrigin 
   * @param translateVectors 
   * 
   * @param straightBulges
   * @return the length of the drawn helix
   * 
   */
  private double drawHelixLikeTemplateHelix(int[] basesInHelixArray, // IN
    ...
    Point2D.Double li = new Point2D.Double(i.x * RNA.LOOP_DISTANCE,
    ...
    for (int k = 0; k < n; k++) {
      ...
      boolean bulge = k >= 1
          && (basesInHelixArray[k] != basesInHelixArray[k - 1] + 1
      ...
      if (k >= 1) {
        ...
        if (bulge) {
          ...
          double delta1 = estimateBulgeWidth(basesInHelixArray[k - 1],
              basesInHelixArray[k]);
          double delta2 = estimateBulgeWidth(basesInHelixArray[kp],
              basesInHelixArray[kp + 1]);
          ...
          if (coords != null) {
            ...
            for (int side = 0; side < 2; side++) {
              ...
              if (side == 0) {
                ...
                pstart.setLocation(
                    o.x + accDelta.x + i.x * delta
                        + j.x * RNA.BASE_PAIR_DISTANCE,
                    o.y + accDelta.y + i.y * delta
                        + j.y * RNA.BASE_PAIR_DISTANCE);
                pend.setLocation(
                    o.x + accDelta.x + j.x * RNA.BASE_PAIR_DISTANCE,
              ...
              double centerOnBisect = ComputeArcCenter.computeArcCenter(delta,
                  arclen);

              // Should we draw the base on an arc or simply use a line?
              if (centerOnBisect > -1000) {
                Point2D.Double center = new Point2D.Double(
                    pstart.x + is.x * delta / 2 + bisectVect.x * centerOnBisect,
                    pstart.y + is.y * delta / 2
                        + bisectVect.y * centerOnBisect);
                int b = firstBase;
                double len = 0;
                double r = Math.hypot(pstart.x - center.x, pstart.y - center.y);
                double alpha0 = MiscGeom.angleFromVector(pstart.x - center.x,
                    pstart.y - center.y);
                while (b < lastBase) {
                  int l = _listeBases.get(b).getElementStructure();
                  if (b < l && l < lastBase) {
                  ...
                  if (b < lastBase) {
                    coords[b].x = center.x
                        + r * Math.cos(alpha0 + alphasign * len / r);
                    coords[b].y = center.y
                        + r * Math.sin(alpha0 + alphasign * len / r);
                  }
                }
              } else {
                // Draw on a line
                ...
                UnpairedLineCounts counts = countUnpairedLine(firstBase,
                    lastBase);
                double LD = Math
                    .max((delta - counts.nBP * RNA.BASE_PAIR_DISTANCE)
                        / counts.nLD, 0);
              ...
              // If so, use drawLoop() to draw it.
              {
                ...
                while (b < lastBase) {
                  ...
                  if (b < l && l < lastBase) {
                    ...
                    double beta = MiscGeom.angleFromVector(b2pos.x - b1pos.x,
                        b2pos.y - b1pos.y) - Math.PI / 2
                        + (flipped ? Math.PI : 0);
                    Point2D.Double loopCenter = new Point2D.Double(
                        (b1pos.x + b2pos.x) / 2, (b1pos.y + b2pos.y) / 2);
                    rna.drawLoop(b, l, loopCenter.x, loopCenter.y, beta, coords,
                        centers, angles, straightBulges);
                    // If the helix is flipped, we need to compute the symmetric
                    // of the whole loop.
                    if (helix.isFlipped()) {
                      Point2D.Double v = new Point2D.Double(Math.cos(beta),
                          Math.sin(beta));
   ...
   * @param coords 
   * @param centers 
   * @param cancelIfNotGood 
   * @return true if drawn
   */
  private boolean drawOnStraightLine(int firstBase, int lastBase,
                                     Point2D.Double P0, Point2D.Double P3,
    ...
    UnpairedLineCounts counts = countUnpairedLine(firstBase, lastBase);
    double LD = Math.max((vectorNorm - counts.nBP * RNA.BASE_PAIR_DISTANCE)
        / counts.nLD, 0);
   ...
   * http://en.wikipedia.org/wiki/Bezier_curve#Cubic_B.C3.A9zier_curves Here we
   * give this four points and an array of bases indexes (which must be indexes
   * in this RNA sequence) which will be moved to be on the Bezier curve. The
   * bases positions are not changed in fact, instead the coords and centers
   * arrays are modified.
   * @param firstBase 
   * @param lastBase 
   * @param P0 
   ...
   * @param centers 
   * @param cancelIfNotGood 
   * @return true if cancelIfNotGood, draw only if it would look good,
   * otherwise just return false
   */
  private boolean drawOnBezierCurve(int firstBase, int lastBase,
    ...
    double LD = Math.max((curveLength - counts.nBP * RNA.BASE_PAIR_DISTANCE)
        / counts.nLD, 0);
  ...
  private void drawOnEllipse(int firstBase, int lastBase, Point2D.Double P0,
                             Point2D.Double P3, Point2D.Double[] coords,
                             Point2D.Double[] centers, boolean reverse) {

    UnpairedLineCounts counts = countUnpairedLine(firstBase, lastBase);
    double halfEllipseLength = RNA.BASE_PAIR_DISTANCE * counts.nBP
        + RNA.LOOP_DISTANCE * counts.nLD;
    double fullEllipseLength = halfEllipseLength * 2;

    double axisA = P0.distance(P3) / 2;
    double axisB = ComputeEllipseAxis.computeEllipseAxis(axisA,
        fullEllipseLength);
    ...
    if (reverse) {
      ...
      tranform1.scale(1, -1);
      tranform1.transform(sequenceBasesCoords, 0, sequenceBasesCoords, 0,
          sequenceBasesCoords.length);
    }

    // Translate + rotate such that ellipse is at the right place
    AffineTransform tranform = HalfEllipse.matchAxisA(P0, P3);
    tranform.transform(sequenceBasesCoords, 0, sequenceBasesCoords, 0,
        sequenceBasesCoords.length);
   ...
   * 
   * Argument reverse says if we should reverse the direction of inserted
   * helices.
   * @param firstBase 
   * @param lastBase 
   * @param P0 
   ...
   * @param straightBulges 
   */
  private void drawAlongCurve(int firstBase, int lastBase, Point2D.Double P0,
                              Point2D.Double P1, Point2D.Double P2,
                              Point2D.Double P3, Point2D.Double[] coords,
                              Point2D.Double[] centers, double[] angles,
                              DrawRNATemplateCurveMethod curveMethod,
                              boolean reverse, boolean straightBulges) {
    ...
    for (int depth = 0, i = firstBase; i <= lastBase; i++) {
      ...
      if (k < 0 || k > lastBase || k < firstBase) {
        if (depth == 0) {
          alongBezierCurve.add(Integer.valueOf(i));
        }
      } else {
        if (i < k) {
          if (depth == 0) {
            alongBezierCurve.add(Integer.valueOf(i));
            alongBezierCurve.add(Integer.valueOf(k));
          }
          depth++;
        } else {
          depth--;
    ...
    int[] alongBezierCurveArray = RNATemplateAlign
        .intArrayFromList(alongBezierCurve);
    if (n > 0) {
      if (curveMethod == DrawRNATemplateCurveMethod.ALWAYS_REPLACE_BY_ELLIPSES) {
        ...
        boolean passed;
        if (P1 != null && P2 != null) {
          passed = drawOnBezierCurve(firstBase, lastBase, P0, P1, P2, P3,
              coords, centers, true);
        } else {
          passed = drawOnStraightLine(firstBase, lastBase, P0, P3, coords,
              centers, true);
        }
        if (!passed) {
          drawOnEllipse(firstBase, lastBase, P0, P3, coords, centers, reverse);
        ...
        if (P1 != null && P2 != null) {
          drawOnBezierCurve(firstBase, lastBase, P0, P1, P2, P3, coords,
              centers, false);
        } else {
          drawOnStraightLine(firstBase, lastBase, P0, P3, coords, centers,
              false);
        }
      }
    }
    ...
    for (int k = 0; k < n - 1; k++) {
      ...
      if (_listeBases.get(b1).getElementStructure() == b2) {
        ...
        double alpha = MiscGeom.angleFromVector(b2pos.x - b1pos.x,
            b2pos.y - b1pos.y);
        rna.drawLoop(b1, b2, (b1pos.x + b2pos.x) / 2, (b1pos.y + b2pos.y) / 2,
            alpha - Math.PI / 2, coords, centers, angles, straightBulges);
        if (reverse) {
          Point2D.Double symAxisVect = new Point2D.Double(
              coords[b2].x - coords[b1].x, coords[b2].y - coords[b1].y);
  ...
  /**
   * Compute the symmetric of all the points from first to last in the points
   * array relative to the line that goes through p and has director vector v.
   ...
   * @param last 
   */
  private static void symmetric(Point2D.Double p, Point2D.Double v,
                                Point2D.Double[] points, int first, int last) {
    // ||v||^2
    double lv = v.x * v.x + v.y * v.y;
    for (int i = first; i <= last; i++) {
      // A is the coordinates of points[i] after moving the origin at p
      Point2D.Double A = new Point2D.Double(points[i].x - p.x,
          points[i].y - p.y);
      // Symmetric of A
  ...
  private void computeHelixTranslations(Tree<RNANodeValueTemplate> tree, // IN
    ...
    if (nvt instanceof RNANodeValueTemplateBasePair) {
      ...
      if (!translateVectors.containsKey(helix)) {
        ...
        if (mapping.getAncestor(helix) != null) {
          basesInHelixArray = RNATemplateAlign
              .intArrayFromList(mapping.getAncestor(helix));
          Arrays.sort(basesInHelixArray);
        } else {
          basesInHelixArray = new int[0];
        }
        Point2D.Double helixDeltaVector = computeLengthIncreaseDelta(
            basesInHelixArray, helix, straightBulges);
        newDeltaVector = new Point2D.Double(
            parentDeltaVector.x + helixDeltaVector.x,
            parentDeltaVector.y + helixDeltaVector.y);
      }
    }
    for (Tree<RNANodeValueTemplate> subtree : tree.getChildren()) {
      computeHelixTranslations(subtree, translateVectors, mapping,
          newDeltaVector, straightBulges);
    }
  }

  private Map<RNATemplateHelix, Point2D.Double> computeHelixTranslations(Tree<RNANodeValueTemplate> tree, // IN
                                                                         RNATemplateMapping mapping, // IN
                                                                         boolean straightBulges) {
    Map<RNATemplateHelix, Point2D.Double> translateVectors = new HashMap<RNATemplateHelix, Point2D.Double>();
    computeHelixTranslations(tree, translateVectors, mapping,
        new Point2D.Double(0, 0), straightBulges);
