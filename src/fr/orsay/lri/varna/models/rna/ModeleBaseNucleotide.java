...
public class ModeleBaseNucleotide extends ModeleBase {
   ...
   *        The index of this base
   * @param baseNumber
   *        The number of this base, which may differ from the index (e.g.
  ...
  /**
   * Creates a new rna base with a space as the nitrogenous base and the display
   * style
   * 
   * @param coord
   * @param index
  ...
  public ModeleBaseNucleotide(Point2D.Double coord, int index) {
    this(new Point2D.Double(coord.getX(), coord.getY()), new Point2D.Double(),
  ...
  public ModeleBaseNucleotide(ModeleBaseNucleotide mb, int index) {
    this(new Point2D.Double(mb.getCoords().getX(), mb.getCoords().getY()),
        new Point2D.Double(mb.getCenter().getX(), mb.getCenter().getY()), true,
        mb.getBase(), mb.getStyleBase(), mb.getElementStructure(), index);
  }
  ...
  private enum STATE_SPECIAL_CHARS_STATES {
    NORMAL, SUBSCRIPT, SUPERSCRIPT, COMMAND
  }
    ...
    for (int i = 0; i < s.length(); i++) {
      ...
      case SUPERSCRIPT: {
        ...
        case '\\':
          ...
          break;
        default:
          if ((state == STATE_SPECIAL_CHARS_STATES.SUBSCRIPT)
              && _subscripts.containsKey(c))
            result += _subscripts.get(c);
          else if ((state == STATE_SPECIAL_CHARS_STATES.SUPERSCRIPT)
              && _superscripts.containsKey(c))
