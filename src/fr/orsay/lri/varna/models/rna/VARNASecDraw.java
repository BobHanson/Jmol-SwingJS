...
public class VARNASecDraw {
  ...
  public class PairedPortion extends Portion {
    ...
    @Override
    public String toString() {
      return "H[" + _pos1 + "," + (_pos1 + _len - 1) + "][" + (_pos2 - _len + 1)
          + "," + (_pos2) + "]\n" + _r.toString();
    }
  ...
  private void buildTree(int i, int j, RNATree parent, RNA r) {
    ...
    if (r.get_listeBases().get(i).getElementStructure() == j) {
      ...
      int j1 = j;
      boolean over = false;
      while ((i + 1 < r.get_listeBases().size()) && (j - 1 >= 0)
          && (i + 1 <= j - 1) && !over) {
        if (r.get_listeBases().get(i).getElementStructure() != j) {
          over = true;
        } else {
  ...
  */
  ...
  public void drawTree(double x0, double y0, RNATree t, double dir, RNA r)
      throws Exception {
    boolean collision = true;
    double x = x0;
    double y = y0;
    ...
    int nbHel = 1;
    int nbUn = 0;
    double totCirc = RNA.BASE_PAIR_DISTANCE + RNA.LOOP_DISTANCE;
    for (int i = 0; i < t.getNumPortions(); i++) {
      Portion p = t.getPortion(i);
      if (p instanceof PairedPortion) {
        totCirc += (RNA.BASE_PAIR_DISTANCE + RNA.LOOP_DISTANCE);
        nbHel += 1;
      } else {
        totCirc += RNA.LOOP_DISTANCE * (p.getNumBases());
    ...
    double inc = MAX_NUM_DIR / ((double) numHelices + 1);
    double val = inc;
    ...
    System.out.println();
    double angleIncr = 2.0 * Math.PI / MAX_NUM_DIR;
    while (collision) {
      x = x0 + radius * Math.cos(dir + Math.PI);
      y = y0 + radius * Math.sin(dir + Math.PI);
      ArrayList<GeneralPath> shapes = new ArrayList<GeneralPath>();
      ...
      for (int i = 0; i < t.getNumPortions(); i++) {
        ...
        if (p instanceof PairedPortion) {
          ...
          for (int j = 0; j < pp._len; j++) {
            ...
            double vx = Math.cos(ndir);
            double vy = Math.sin(ndir);
            double nx = x + (((j) * RNA.LOOP_DISTANCE + radius) * vx);
            double ny = y + (((j) * RNA.LOOP_DISTANCE + radius) * vy);
            r.get_listeBases().get(i1).setCoords(
                new Point2D.Double(nx + RNA.BASE_PAIR_DISTANCE * vy / 2.0,
                    ny - RNA.BASE_PAIR_DISTANCE * vx / 2.0));
            r.get_listeBases().get(i2).setCoords(
                new Point2D.Double(nx - RNA.BASE_PAIR_DISTANCE * vy / 2.0,
                    ny + RNA.BASE_PAIR_DISTANCE * vx / 2.0));
          }
          double nx = x
              + (((pp._len - 1) * RNA.LOOP_DISTANCE + radius) * Math.cos(ndir));
          double ny = y
              + (((pp._len - 1) * RNA.LOOP_DISTANCE + radius) * Math.sin(ndir));
          ...
          for (int j = 0; j < up._len; j++) {
            /*ndir = dir + circ*angleIncr;
            	...
            	r.get_listeBases().get(up._pos+j).set_coords(new Point2D.Double(nx,ny));
            	circ += mod*r.LOOP_DISTANCE;*/
            r.get_listeBases().get(up._pos + j)
  ...
  private class HelixEmbedding {
    ...
    public HelixEmbedding(Point2D.Double support, PairedPortion p, RNA r,
        HelixEmbedding parent) {
      _support = support;
    ...
    public void addHelixEmbedding(HelixEmbedding h, int index) {
      _children.add(h);
      _indices.add(Integer.valueOf(index));
    }
      ...
      if (_parent._children.size() < VARNASecDraw.MAX_NUM_DIR - 1) {
        if (_parent._children.size() == 1) {
          ...
          if (i == 0) {
            ...
            min = _parent._indices.get(i - 1).intValue() + 1;
          }
          if (i == _parent._children.size() - 1) {
            max = VARNASecDraw.MAX_NUM_DIR - 1;
          } else {
            max = _parent._indices.get(i + 1).intValue() - 1;
          }
        }
        int prevIndex = _parent._indices.get(i).intValue();
        int newIndex = min + _rnd.nextInt(max + 1 - min);
        double rot = ((newIndex - prevIndex) * Math.PI * 2.0)
            / MAX_NUM_DIR;
        _parent._indices.set(i, Integer.valueOf(newIndex));
        rotate(rot);
    ...
    public void cancelMove(int delta) {
      int i = _parent._children.indexOf(this);
      int prevIndex = _parent._indices.get(i).intValue();
      double rot = ((-delta) * Math.PI * 2.0) / MAX_NUM_DIR;
      _parent._indices.set(i, Integer.valueOf(prevIndex - delta));
      rotate(rot);
    }

    public void rotate(double angle) {
      transform(
          AffineTransform.getRotateInstance(angle, _support.x, _support.y));
    }
    ...
    public void reflectCoordinates() {
      ...
      if (_p._len > 0) {
        PathIterator pi = _clip
            .getPathIterator(AffineTransform.getRotateInstance(0.0));
...
//        Point2D.Double endRight = p.get(2);
  ...
  //        System.out.println("DELTA: " + interval + " " + RNA.LOOP_DISTANCE);
        }
        for (int n = 0; n < _p._len; n++) {
          int i = _p._pos1 + n;
          int j = _p._pos2 - n;
          ModeleBase mbLeft = mbl.get(i);
          mbLeft.setCoords(new Point2D.Double(startLeft.x + n * vx * interval,
              startLeft.y + n * vy * interval));
          ModeleBase mbRight = mbl.get(j);
          mbRight.setCoords(new Point2D.Double(startRight.x + n * vx * interval,
              startRight.y + n * vy * interval));
        }
      }
      for (int i = 0; i < _children.size(); i++) {
    ...
    private void placeTerminalLoop(ArrayList<ModeleBase> mbl) {
      if ((_children.size() == 0) && (_p._r.getNumPortions() == 1)) {
        ...
        if (p instanceof UnpairedPortion) {
          UnpairedPortion up = (UnpairedPortion) p;
          double rad = determineRadius(1, up.getNumBases());
          ...
          Point2D.Double pm = new Point2D.Double((pl.x + pr.x) / 2.0,
              (pl.y + pr.y) / 2.0);
          double vx = (pl.x - pr.x) / pl.distance(pr);
          double vy = (pl.y - pr.y) / pl.distance(pr);
          double vnx = -vy, vny = vx;
          Point2D.Double pc = new Point2D.Double(pm.x + rad * vnx,
              pm.y + rad * vny);
          double circ = RNA.LOOP_DISTANCE * (1.0 + up.getNumBases())
              + RNA.BASE_PAIR_DISTANCE;
          double incrLoop = Math.PI * 2.0 * RNA.LOOP_DISTANCE / circ;
          double angle = Math.PI * 2.0 * RNA.BASE_PAIR_DISTANCE / (2.0 * circ);
          for (int j = 0; j < up._len; j++) {
            int n = up._pos + j;
            ...
            Point2D.Double pf = new Point2D.Double(pc.x + rad * dx,
                pc.y + rad * dy);
            mb.setCoords(pf);
          }
  ...
  public static double determineRadius(int numHelices, int numUnpaired) {
    double circ = numHelices * RNA.BASE_PAIR_DISTANCE
        + (numHelices + numUnpaired) * RNA.LOOP_DISTANCE;
    return circ / (2.0 * Math.PI);
  }

  public void predrawTree(double x0, double y0, RNATree t, double dir, RNA r,
                          HelixEmbedding parent, ArrayList<HelixEmbedding> all)
      throws Exception {
    double x = x0;
    double y = y0;
    int numHelices = 0;
...
//    double totCirc = RNA.BASE_PAIR_DISTANCE + RNA.LOOP_DISTANCE;
  ...
  //      totCirc += (RNA.BASE_PAIR_DISTANCE + RNA.LOOP_DISTANCE);
    ...
    double radius = determineRadius(numHelices + 1, numUBases);
    ...
    for (int i = 0; i < t.getNumPortions(); i++) {
      ...
      if (p instanceof PairedPortion) {
        ...
        double ny = y
            + (((pp._len - 1) * RNA.LOOP_DISTANCE + radius) * Math.sin(ndir));
        HelixEmbedding nh = new HelixEmbedding(new Point2D.Double(x, y), pp, r,
            parent);
  ...
  public RNATree drawRNA(double dirAngle, RNA r) {
    ...
    try {
      HelixEmbedding root = new HelixEmbedding(new Point2D.Double(0.0, 0.0),
          new PairedPortion(0, 0, 0, t), r, null);
