...
public class ModeleColorMap implements Cloneable, Serializable {
  ...
  public void addColor(double val, Color col) {
    int offset = Arrays.binarySearch(_values.toArray(), Double.valueOf(val));
    if (offset < 0) {
      int inspoint = (-offset) - 1;
      _map.insertElementAt(col, inspoint);
      _values.insertElementAt(Double.valueOf(val), inspoint);
    }
  }

  public double getMinValue() {
    if (_values.size() > 0)
      return _values.get(0).doubleValue();
    return 0.0;
  }

  public double getMaxValue() {
    if (_values.size() > 0)
      return _values.get(_values.size() - 1).doubleValue();
    return 0.0;
  }
  ...
  public Color getColorForValue(double val) {
    ...
    if (val <= getMinValue()) {
      ...
      if (offset >= 0) {
        ...
        double v1 = _values.get(inspoint).doubleValue();
        if (inspoint > 0) {
          Color c2 = _map.get(inspoint - 1);
          double v2 = _values.get(inspoint - 1).doubleValue();
          double blendCoeff = (v2 - val) / (v2 - v1);
          result = new Color(
              (int) (blendCoeff * c1.getRed()
                  + (1.0 - blendCoeff) * c2.getRed()),
              (int) (blendCoeff * c1.getGreen()
                  + (1.0 - blendCoeff) * c2.getGreen()),
              (int) (blendCoeff * c1.getBlue()
                  + (1.0 - blendCoeff) * c2.getBlue()));
  ...
  public void rescale(double newMin, double newMax) {
    ...
    if (newMax != newMin) {
      ...
      for (int i = 0; i < _values.size(); i++) {
        double valBck = _values.get(i).doubleValue();
        _values.set(i, Double.valueOf(
            newMin + (newMax - newMin) * (valBck - minBck) / (spanBck)));
      }
    }
  }
  ...
  public boolean equals(ModeleColorMap cm) {
    ...
    for (int i = 0; i < getNumColors(); i++) {
      if ((!getColorAt(i).equals(cm.getColorAt(i)))
          || (!getValueAt(i).equals(cm.getValueAt(i))))
        return false;
    }
    return true;
  ...
  public String getParamEncoding() {
    ...
    for (int i = 0; i < getNumColors(); i++) {
      if (i != 0)
        f.format(",");
      f.format("%.2f:%s", _values.get(i), toHex(_map.get(i)));
    }
  ...
  public static String toHex(Color c) {
    ...
    appendHex2(s, c.getRed());
    appendHex2(s, c.getGreen());
    appendHex2(s, c.getBlue());
    return s.toString();
  }

  private static void appendHex2(StringBuffer sb, int i) {
    if (i < 16)
      sb.append("0");
  ...
  public static Color getBgContrast(Color c) {
    Color g = mapContrast.get(c);
    if (g == null) {
      int gray = toFFGGGfromRGB(c.getRGB()) & 0xFF;
      mapContrast.put(c, g = (gray < 128 ? Color.WHITE : Color.BLACK));
    }
    return g;
  }
