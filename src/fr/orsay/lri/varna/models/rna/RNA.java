...
public class RNA extends InterfaceVARNAObservable implements Serializable {
   ...
   * circle and draws the base-pairings as chords of the circle graph.
   */

  public static final int DRAW_MODE_CIRCULAR = 1;
  /**
   * Selects the "tree drawing" algorithm. Draws each loop on a circle whose
   * radius depends on the number of bases involved in the loop. As some helices
   * can be overlapping in the result, basic interaction is provided so that the
   * user can "disentangle" the drawing by spinning the helices around the axis
   * defined by their multiloop (bulge or internal loop) origin. This is roughly
   * the initial placement strategy of RNAViz.
   * 
  ...
  /**
   * Selects the linear algorithm. -- one line of atoms, with hemispherical
   * joins
   */
  ...
  public Color getBaseInnerColor(int i, VARNAConfig conf) {
    ...
    if (conf._drawColorMap) {
      ...
      result = conf._dashBasesColor;
        ...
        && !NormalBases.contains(res.toLowerCase()))) {
  ...
  public Color getBasePairColor(ModeleBP bp, VARNAConfig conf) {
    ...
    if (conf._useBaseColorsForBPs) {
      bondColor = _listeBases.get(bp.getPartner5().getIndex()).getStyleBase()
          .getBaseInnerColor();
    }
    if (bp != null) {
  ...
  private void drawSymbol(SecStrDrawingProducer out, double posx, double posy,
                          double normx, double normy, double radius,
  ...
  private void drawBasePairArc(SecStrDrawingProducer out, int i, int j,
                               Point2D.Double orig, Point2D.Double dest,
                               ModeleBP style, VARNAConfig conf) {
    double coef;
    double distance;
    Point2D.Double center = new Point2D.Double((orig.x + dest.x) / 2.,
        (orig.y + dest.y) / 2. + BASE_RADIUS);
    if (j - i == 1)
      ...
      if (style.isCanonical()) {
        if (style.isCanonicalGC()) {
          ...
          drawSymbol(out, center.x, center.y + distance * coef / 2., 180., 0,
              radiusCircle, style.isCIS(), style.getEdgePartner5(), thickness);
        } else {
  ...
  private void drawBasePair(SecStrDrawingProducer out, Point2D.Double orig,
                            Point2D.Double dest, ModeleBP style,
                            VARNAConfig conf) {
    ...
    double nx = -dy;
    double ny = dx;
    orig = new Point2D.Double(orig.x + BASE_RADIUS * dx,
        orig.y + BASE_RADIUS * dy);
    dest = new Point2D.Double(dest.x - BASE_RADIUS * dx,
        dest.y - BASE_RADIUS * dy);
      ...
      if (style.isCanonical()) {
        if (style.isCanonicalGC()) {
          if ((orig.x != dest.x) || (orig.y != dest.y)) {
            nx *= BASE_RADIUS / 4.0;
            ny *= BASE_RADIUS / 4.0;
            out.drawLine((orig.x + nx), (orig.y + ny), (dest.x + nx),
                (dest.y + ny), conf._bpThickness);
            out.drawLine((orig.x - nx), (orig.y - ny), (dest.x - nx),
                (dest.y - ny), conf._bpThickness);
          }
          ...
          out.drawLine(orig.x, orig.y, dest.x, dest.y, conf._bpThickness);
          drawSymbol(out, cx, cy, nx, ny, radiusCircle, false, ModeleBP.Edge.WC,
              thickness);
        }
          ...
          double cy = (dest.y + orig.y) / 2.0;
          out.drawLine(orig.x, orig.y, dest.x, dest.y, conf._bpThickness);
          drawSymbol(out, cx, cy, nx, ny, radiusCircle, style.isCIS(),
        ...
        out.drawLine(orig.x, orig.y, dest.x, dest.y, conf._bpThickness);
        if (p1 == p2) {
          drawSymbol(out, cx, cy, nx, ny, radiusCircle, style.isCIS(), p1,
              thickness);
        } else {
  ...
  private void drawColorMap(VARNAConfig _conf, SecStrDrawingProducer out) {
    ...
    double xBase = (currentBBox.getMaxX() - _conf._colorMapWidth
        - _conf._colorMapXOffset);
    // double yBase = (minY - _conf._colorMapHeight +
    // _conf._colorMapYOffset);
    double yBase = (currentBBox.getMinY() - _conf._colorMapHeight
        - VARNAConfig.DEFAULT_COLOR_MAP_FONT_SIZE);

    for (int i = 0; i < _conf._colorMapWidth; i++) {
      double ratio = ((i) / (_conf._colorMapWidth - 1));
    ...
    out.drawRectangle(xBase, yBase,
        _conf._colorMapWidth + VARNAConfig.DEFAULT_COLOR_MAP_STRIPE_WIDTH - 1,
  ...
  private void saveRNA(String path, VARNAConfig conf, double scale,
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      ...
      minY = Math.min(minY,
          -(_listeBases.get(i).getCoords().getY() - BASE_RADIUS - EPSMargin));
      maxX = Math.max(maxX,
          (_listeBases.get(i).getCoords().getX() + BASE_RADIUS + EPSMargin));
      maxY = Math.max(maxY,
          -(_listeBases.get(i).getCoords().getY() + BASE_RADIUS + EPSMargin));
    ...
    out.setFont(SecStrDrawingProducer.FONT_HELVETICA_BOLD, baseFontSize);
      ...
      if (_listeBases.get(i) instanceof ModeleBasesComparison) {
        ...
        if (conf._fillBases) {
          out.fillRectangle(x0 - 1.5 * BASE_RADIUS, y0 - BASE_RADIUS,
              3 * BASE_RADIUS, 2 * BASE_RADIUS, baseInnerColor);
        }
    ...
    double numFontSize = 1.5 * BASE_RADIUS;
    out.setFont(SecStrDrawingProducer.FONT_HELVETICA_BOLD, numFontSize);

    for (int i = 0; i < _listeBases.size(); i++) {
      int basenum = _listeBases.get(i).getResidueNumber();
      if (basenum == -1) {
        basenum = i + 1;
      }
      ...
      if (this.isNumberDrawn(mb, conf._numPeriod)) {
        ...
        dx /= norm;
        dy /= norm;
        Point2D.Double vn = VARNAPanel.computeExcentricUnitVector(i, coords,
            centers);

        out.drawLine((x0 + 1.5 * BASE_RADIUS * vn.x),
            (y0 + 1.5 * BASE_RADIUS * vn.y), (x0 + 2.5 * BASE_RADIUS * vn.x),
            (y0 + 2.5 * BASE_RADIUS * vn.y), 1);
        out.drawText((x0 + (conf._distNumbers + 1.0) * BASE_RADIUS * vn.x),
  ...
  private void drawChemProbAnnotation(SecStrDrawingProducer out,
                                      ...
                                      Point2D.Double anchor, double minX,
                                      double minY) {
    out.setColor(cpa.getColor());
    Point2D.Double v = cpa.getDirVector();
    Point2D.Double vn = cpa.getNormalVector();
    Point2D.Double base = new Point2D.Double((anchor.x + CHEM_PROB_DIST * v.x),
        (anchor.y + CHEM_PROB_DIST * v.y));
    ...
    case ARROW: {
      ...
      Point2D.Double arrowTip2 = new Point2D.Double(
          (base.x + cpa.getIntensity()
              * (-CHEM_PROB_ARROW_WIDTH * vn.x + CHEM_PROB_ARROW_HEIGHT * v.x)),
          (base.y + cpa.getIntensity() * (-CHEM_PROB_ARROW_WIDTH * vn.y
              + CHEM_PROB_ARROW_HEIGHT * v.y)));
      out.drawLine(base.x - minX, minY - base.y, edge.x - minX, minY - edge.y,
          thickness);
      out.drawLine(base.x - minX, minY - base.y, arrowTip1.x - minX,
          minY - arrowTip1.y, thickness);
      out.drawLine(base.x - minX, minY - base.y, arrowTip2.x - minX,
          minY - arrowTip2.y, thickness);
    }
      break;
    case PIN: {
    ...
    case DOT: {
      ...
      Point2D.Double center = new Point2D.Double((base.x + radius * v.x) - minX,
          minY - (base.y + radius * v.y));
  ...
  private void renderAnnotations(SecStrDrawingProducer out, double minX,
    ...
    for (TextAnnotation textAnnotation : getAnnotations()) {
      ...
      out.setFont(SecStrDrawingProducer.FONT_HELVETICA_BOLD,
          2.0 * textAnnotation.getFont().getSize());
  ...
  public Rectangle2D.Double getBBox() {
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      minx = Math.min(_listeBases.get(i).getCoords().getX() - BASE_RADIUS,
          minx);
      miny = Math.min(_listeBases.get(i).getCoords().getY() - BASE_RADIUS,
          miny);
      maxx = Math.max(_listeBases.get(i).getCoords().getX() + BASE_RADIUS,
          maxx);
      maxy = Math.max(_listeBases.get(i).getCoords().getY() + BASE_RADIUS,
          maxy);
    }
    result.x = minx;
    result.y = miny;
  ...
  public int getBaseNumber(int i) {
    if ((i >= 0) && (i < _listeBases.size())) {
      return _listeBases.get(i).getResidueNumber();
    }
    return -1;
  }
  ...
  public void drawRNACircle(VARNAConfig conf) {
    ...
    int radius = (int) ((3 * (_listeBases.size() + 1) * BASE_RADIUS)
        / (2 * Math.PI));
    double angle;
    for (int i = 0; i < _listeBases.size(); i++) {
      angle = -(((-(i + 1)) * 2.0 * Math.PI) / (_listeBases.size() + 1)
          - Math.PI / 2.0);
      _listeBases.get(i)
          .setCoords(new Point2D.Double(
  ...
  public void drawRNALine(VARNAConfig conf) {
    ...
    for (int i = 0; i < get_listeBases().size(); i++) {
      get_listeBases().get(i)
          .setCoords(new Point2D.Double(i * conf._spaceBetweenBases * 20, 0));
      get_listeBases().get(i)
          .setCenter(new Point2D.Double(i * conf._spaceBetweenBases * 20, -10));
    }
  }
  ...
  public RNATemplateMapping drawRNATemplate(RNATemplate template,
    ...
    return drawRNATemplate(template, conf, DrawRNATemplateMethod.getDefault(),
  ...
  public RNATemplateMapping drawRNATemplate(RNATemplate template,
    ...
    drawRNATemplate.drawRNATemplate(template, conf, helixLengthAdjustmentMethod,
        curveMethod, straightBulges);
    return drawRNATemplate.getMapping();
  }

  private static double objFun(int n1, int n2, double r, double bpdist,
                               double multidist) {
    return ((n1) * 2.0 * Math.asin((bpdist) / (2.0 * r))
        + (n2) * 2.0 * Math.asin((multidist) / (2.0 * r)) - (2.0 * Math.PI));
  }

  public double determineRadius(int nbHel, int nbUnpaired, double startRadius) {
    return determineRadius(nbHel, nbUnpaired, startRadius, BASE_PAIR_DISTANCE,
        MULTILOOP_DISTANCE);
  }
  ...
  public static final double[] HYSTERESIS_ATTRACTORS = { 0., Math.PI / 4.,
      Math.PI / 2., 3. * Math.PI / 4., Math.PI, 5. * (Math.PI) / 4.,
  ...
  public static double correctHysteresis(double angle) {
    ...
    for (int i = 0; i < HYSTERESIS_ATTRACTORS.length; i++) {
      ...
      if (Math
          .abs(normalizeAngle(att - result, -Math.PI)) < HYSTERESIS_EPSILON) {
        result = att;
      }
    }
    return result;
  ...
  private void distributeUnpaired(double radius, double angle, double pHel,
                                  double base, Point2D.Double center,
    ...
    Point2D.Double PA = new Point2D.Double(
        center.x + radius * Math.cos(base + pHel),
    ...
    Point2D.Double VN = new Point2D.Double((PB.y - PA.y) / dist,
        (-PB.x + PA.x) / dist);
    if (mydist < 2 * BASE_RADIUS) {
      addedRadius = Math.min(1.0, (2 * BASE_RADIUS - mydist) / 4)
          * computeRadius(mydist,
              2.29 * (bases.size() + 1) * BASE_RADIUS - mydist);
    }

    ArrayList<Point2D.Double> pos = computeNewAngles(bases.size(), center, VN,
        angle, base + pHel, radius, addedRadius);
    for (int i = 0; i < bases.size(); i++) {
      int k = bases.get(i).intValue();
  ...
  private double computeRadius(double b, double pobj) {
    ...
    double p = Math.PI * (a + b) * (1 + h / 4. + h * h / 64. + h * h * h / 256.
        + 25. * h * h * h * h / 16384.) / 2.0;
    double aold = a + 1.;
    while ((Math.abs(p - pobj) > 10e-4) && (aold != a)) {
      ...
      h = (a - b) * (a - b) / ((a + b) * (a + b));
      p = (Math.PI * (a + b) * (1 + h / 4. + h * h / 64. + h * h * h / 256.
          + 25. * h * h * h * h / 16384.)) / 2.0;
    }
    return a;
  }
  ...
  private void fixUnpairedPositions(boolean isDirect, double angleRightPartner,
                                    double angleLimitLeft,
                                    double angleLimitRight,
                                    double angleLeftPartner, double radius,
                                    double base, Point2D.Double center,
    ...
    if (isDirect) {
      ...
      distributeUnpaired(radius, anglePrev, angleRightPartner, base, center,
          prevBases);
      distributeUnpaired(radius, -angleNext, angleLeftPartner, base, center,
          nextBases);
    } else {
      ...
      distributeUnpaired(radius, -anglePrev, angleLeftPartner, base, center,
          prevBases);
      distributeUnpaired(radius, angleNext, angleRightPartner, base, center,
          nextBases);
    }

  }

  private static Point2D.Double getPoint(double angleLine, double angleBulge,
                                         Point2D.Double center,
                                         Point2D.Double VN, double radius,
                                         double addedRadius, double dirBulge) {
    return new Point2D.Double(
  ...
  private ArrayList<Point2D.Double> computeNewAngles(int numPoints,
                                                     Point2D.Double center,
                                                     Point2D.Double VN,
                                                     double angle,
                                                     double angleBase,
    ...
    if (numPoints > 0) {
      ...
      for (int i = 0; i < numPoints; i++) {
        ...
        Point2D.Double currP = getPoint(angleLine, angleBulge, center, VN,
            radius, addedRadius, dirBulge);

        int numIter = 0;
        while ((Math.abs(currP.distance(prevP) - dtarget) > 0.01)
            && (numIter < 100)) {
          ...
          angleBulge = Math.PI * fact;
          currP = getPoint(angleLine, angleBulge, center, VN, radius,
              addedRadius, dirBulge);
          numIter++;
        }
        factors.add(Double.valueOf(fact));
        prevP = currP;
      }

      double rescale = 1.0 / (factors.get(factors.size() - 1).doubleValue()
          + factors.get(0).doubleValue());

      for (int j = 0; j < factors.size(); j++) {
        factors.set(j, Double.valueOf(factors.get(j).doubleValue() * rescale));
      }

      if (addedRadius > 0) {
        prevP = getPoint(angleBase, 0, center, VN, radius, addedRadius,
            dirBulge);
        double totDist = 0.0;
        for (int j = 0; j < factors.size(); j++) {
          double newfact = factors.get(j).doubleValue();
        ...
        totDist += getPoint(angleBase + angle, Math.PI, center, VN, radius,
            addedRadius, dirBulge).distance(prevP);
        dtarget = totDist / (numPoints + 1);
        fact = 0.0;
        factors = new ArrayList<Double>();
        prevP = new Point2D.Double(center.x + radius * Math.cos(angleBase),
            center.y + radius * Math.sin(angleBase));
        for (int i = 0; i < numPoints; i++) {
          double lbound = fact;
          double ubound = 1.5;
        ...
        rescale = 1.0 / (factors.get(factors.size() - 1).doubleValue()
            + factors.get(0).doubleValue());
        for (int j = 0; j < factors.size(); j++) {
          factors.set(j,
              Double.valueOf(factors.get(j).doubleValue() * rescale));
        }
      }
      ...
      for (int j = 0; j < factors.size(); j++) {
        ...
        double angleLine = angleBase + angle * newfact;
        angleBulge = Math.PI * newfact;
        result.add(getPoint(angleLine, angleBulge, center, VN, radius,
            addedRadius, dirBulge));
      }
    }
    return result;
  ...
  void drawLoop(int i, int j, double x, double y, double dirAngle,
                ...
                double[] angles, boolean straightBulges) {
    if (i > j) {
      return;
    }
    ...
    if (_listeBases.get(i).getElementStructure() == j) {
      ...
      coords[i].x = (x
          + BASE_PAIR_DISTANCE * Math.cos(dirAngle - normalAngle) / 2.0);
      coords[i].y = (y
          + BASE_PAIR_DISTANCE * Math.sin(dirAngle - normalAngle) / 2.0);
      coords[j].x = (x
          + BASE_PAIR_DISTANCE * Math.cos(dirAngle + normalAngle) / 2.0);
      coords[j].y = (y
          + BASE_PAIR_DISTANCE * Math.sin(dirAngle + normalAngle) / 2.0);
      drawLoop(i + 1, j - 1, x + LOOP_DISTANCE * Math.cos(dirAngle),
          y + LOOP_DISTANCE * Math.sin(dirAngle), dirAngle, coords, centers,
          angles, straightBulges);
    } else {
      int k = i;
      ...
      if (mlSize > 3) {
        multiLoopRadius = determineRadius(numHelices, mlSize - numHelices,
            (totalLength) / (2.0 * Math.PI), BASE_PAIR_DISTANCE,
            MULTILOOP_DISTANCE);
        angleIncrementML = -2.0
            * Math.asin(((float) MULTILOOP_DISTANCE) / (2.0 * multiLoopRadius));
        angleIncrementBP = -2.0
            * Math.asin(((float) BASE_PAIR_DISTANCE) / (2.0 * multiLoopRadius));
      } else {
      ...
      double centerDist = Math.sqrt(Math.max(
          Math.pow(multiLoopRadius, 2) - Math.pow(BASE_PAIR_DISTANCE / 2.0, 2),
          0.0)) - LOOP_DISTANCE;
      ...
      Couple<Double, Double> currInterval = newCouple(0,
          baseAngle - 1.0 * angleIncrementML);
      ...
      for (k = basesMultiLoop.size() - 1; k >= 0; k--) {
        ...
        boolean isPaired3 = isPaired
            && (_listeBases.get(l).getElementStructure() < l);
        boolean isPaired5 = isPaired && !isPaired3;
        if (isPaired3) {
          if ((numHelices == 2) && straightBulges) {
            baseAngle = dirAngle - angleIncrementBP / 2.;
          } else {
            baseAngle = correctHysteresis(baseAngle + angleIncrementBP / 2.)
                - angleIncrementBP / 2.;
          }
          currInterval.first = Double.valueOf(baseAngle);
          intervals.add(new Couple<ArrayList<Integer>, Couple<Double, Double>>(
              currUnpaired, currInterval));
          currInterval = newCouple(-1, -1);
          currUnpaired = new ArrayList<Integer>();
        } else if (isPaired5) {
          currInterval.second = Double.valueOf(baseAngle);
        } else {
          currUnpaired.add(Integer.valueOf(l));
        }
      ...
      currInterval.first = Double
          .valueOf(dirAngle - Math.PI - 0.5 * angleIncrementBP);
      intervals.add(new Couple<ArrayList<Integer>, Couple<Double, Double>>(
          currUnpaired, currInterval));
      ...
      for (Couple<ArrayList<Integer>, Couple<Double, Double>> inter : intervals) {
        ...
        double mina = inter.second.first.doubleValue();
        double maxa = normalizeAngle(inter.second.second.doubleValue(), mina);
        //System.out.println(""+mina+" " +maxa);

        for (int n = 0; n < inter.first.size(); n++) {
          double ratio = (1. + n) / (1. + inter.first.size());
          int b = inter.first.get(n).intValue();
      ...
      for (k = 0; k < helices.size(); k++) {
        ...
        drawLoop(m + 1, n - 1,
            (LOOP_DISTANCE * Math.cos(newAngle))
                + (coords[m].x + coords[n].x) / 2.0,
            (LOOP_DISTANCE * Math.sin(newAngle))
                + (coords[m].y + coords[n].y) / 2.0,
            newAngle, coords, centers, angles, straightBulges);
      }
    }
  }

  private static Couple<Double, Double> newCouple(double i, double j) {
    return new Couple<Double, Double>(Double.valueOf(i), Double.valueOf(j));
  }

  private static Couple<Integer, Integer> newCoupleI(int i, int j) {
    return new Couple<Integer, Integer>(Integer.valueOf(i), Integer.valueOf(j));
  }
  ...
  public void rotateEverything(double delta, double base, double pLimL,
    ...
    for (int k = h.x; k <= h.y; k++) {
      backupPos.put(Integer.valueOf(k), getBaseAt(k).getCoords());
    }
    ...
    for (int j = 0; j < prevBases.size(); j++) {
      int k = prevBases.get(j).intValue();
      backupPos.put(Integer.valueOf(k), getCoords(k));
    }
    for (int j = 0; j < nextBases.size(); j++) {
      int k = nextBases.get(j).intValue();
      backupPos.put(Integer.valueOf(k), getCoords(k));
    }
    fixUnpairedPositions(isDirect, pHelR, pLimL, pLimR, pHelL, radius, base,
        center, prevBases, nextBases);
  }
  ...
  public void drawRNARadiate(VARNAConfig conf) {
    drawRNARadiate(-1.0, conf._spaceBetweenBases, conf._flatExteriorLoop,
        false);
  }
  ...
  public void drawRNARadiate(double dirAngle, double _spaceBetweenBases,
    ...
    if (flatExteriorLoop) {
      ...
      while (i < _listeBases.size()) {
        ...
        if (j > i) {
          ...
          drawLoop(i, j, x + (BASE_PAIR_DISTANCE * vx / 2.0),
              y + (BASE_PAIR_DISTANCE * vy / 2.0) + increment, dirAngle, coords,
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      _listeBases.get(i).setCoords(new Point2D.Double(
          coords[i].x * _spaceBetweenBases, coords[i].y * _spaceBetweenBases));
      _listeBases.get(i)
          .setCenter(new Point2D.Double(centers[i].x * _spaceBetweenBases,
  ...
  public void drawRNANAView(VARNAConfig conf) throws ExceptionNAViewAlgorithm {
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      pair_table.add(Short
          .valueOf(String.valueOf(_listeBases.get(i).getElementStructure())));
    }
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      _listeBases.get(i)
          .setCoords(new Point2D.Double(
              X.get(i).doubleValue() * 2.5 * conf._spaceBetweenBases,
              Y.get(i).doubleValue() * 2.5 * conf._spaceBetweenBases));
    }
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      ...
      if (indicePartner != -1) {
        ...
        _listeBases.get(i).setCenter(new Point2D.Double(
            (base.x + partner.x) / 2.0, (base.y + partner.y) / 2.0));
      } else {
        Vector<Integer> loop = getLoopBases(i);
        double tmpx = 0.0;
        double tmpy = 0.0;
        for (int j = 0; j < loop.size(); j++) {
          int partner = loop.elementAt(j).intValue();
          Point2D.Double loopmember = _listeBases.get(partner).getCoords();
          tmpx += loopmember.x;
          tmpy += loopmember.y;
        }
        _listeBases.get(i).setCenter(
            new Point2D.Double(tmpx / loop.size(), tmpy / loop.size()));
      }
    }
  }
  ...
  public void setRNA(String seq, String str)
    ...
    for (int i = 0; i < al.size(); i++) {
      if (sepPos.contains(Integer.valueOf(i))
          && al.get(i).equals(DBNStrandSep)) {
        resSepPos.add(Integer.valueOf(alRes.size() - 1));
      } else {
        alRes.add(al.get(i));
        if (i < str.length()) {
          strRes += str.charAt(i);
   ...
   * owning by each RNA alignment and both.
   * 
   * @param seq
   ...
   *        1 when first RNA alignment base, 2 when second RNA alignment base)
   * @throws ExceptionUnmatchedClosingParentheses
  ...
  public void setRNA(String seq, String struct, ArrayList<Integer> basesOwn)
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (seq.charAt(j) != seq.charAt(j + 1)) {
        ModeleBasesComparison mbc = new ModeleBasesComparison(seq.charAt(j),
            seq.charAt(j + 1), i);
        mbc.set_appartenance(basesOwn.get(i).intValue());
        mbc.setResidueNumber(i + 1);
        mb = mbc;
      } else {
  ...
  public ArrayList<Integer> getHelix(int index) {
    ...
    for (int i = p.x; i <= p.y; i++) {
      ...
      result.add(Integer.valueOf(_listeBases.get(i).getElementStructure()));
    }
    return result;
  }
  ...
  public Vector<Integer> getLoopBases(int startIndex) {
    ...
    result.add(Integer.valueOf(startIndex));
    ...
    for (int i = 0; i < rankedBPs.size(); i++) {
      ...
      if (!lbp.isEmpty()) {
        ...
        if (ok) {
          lbp.remove(0);
          currentBPs.add(bp);
          pile.add(Integer.valueOf(bp.getIndex3()));
        }
      }
  ...
  public String getStructCT() {
    String result = "";
    for (int i = 0; i < _listeBases.size(); i++) {
      result += (i + 1) + " " + _listeBases.get(i).getContent() + " " + i + " "
          + (i + 2) + " " + (_listeBases.get(i).getElementStructure() + 1) + " "
          + (i + 1) + "\n";
    }
    return result;
  }
  ...
  public void saveAsCT(String path, String title) throws ExceptionExportFailed {
    try {
      FileWriter f = new FileWriter(path);
  ...
  public void applyStyleOnBases(ArrayList<Integer> basesList,
    ...
    for (int i = 1; i < basesList.size(); i++) {
      _listeBases.get(basesList.get(i).intValue()).setStyleBase(style);
    }
  }
  ...
  public ArrayList<Integer> findPair(int indice) {
    ...
    if (j != -1) {
      base.add(Integer.valueOf(Math.min(indice, j)));
      base.add(Integer.valueOf(Math.max(indice, j)));
    }
  ...
  public ArrayList<Integer> findHelix(int indice) {
    ...
    if (get_listeBases().get(indice).getElementStructure() != -1) {
      ...
      list.add(
          Integer.valueOf(get_listeBases().get(indice).getElementStructure()));
      ...
      while (indice + i < get_listeBases().size()
          ...
          && get_listeBases().get(indice + i).getElementStructure() == prec
              - 1) {
        list.add(Integer.valueOf(indice + i));
        list.add(Integer
            .valueOf(get_listeBases().get(indice + i).getElementStructure()));
  ...
  public ArrayList<Integer> findNonPairedBaseGroup(int nearestBase) {
    // detection 3', 5', bulge
    ArrayList<Integer> list = new ArrayList<Integer>();
    int indice = nearestBase;
  ...
  /**
   * Translates a base number into its corresponding index. Although both should
  ...
  /**
   * Adds a base pair to this RNA's structure. Tries to add it to the secondary
   * structure first, eventually adding it to the 'tertiary' interactions if it
   * clashes with the current secondary structure.
   * 
   * @param baseNumber5
  ...
  /**
   * Adds a base pair to this RNA's structure. Tries to add it to the secondary
   * structure first, possibly adding it to the 'tertiary' interactions if it
   * clashes with the current secondary structure.
   * 
   * @param number5
   *        - Base number of the origin of this base pair
  ...
  public void autoAnnotateStrandEnds() {
    if (!_strandEndsAnnotated) {
      ...
      for (int i = 0; i < _listeBases.size() - 1; i++) {
        int realposA = _listeBases.get(i).getResidueNumber();
        int realposB = _listeBases.get(i + 1).getResidueNumber();
      ...
      if (!endAnnotate) {
        addAnnotation(
            new TextAnnotation("3'", _listeBases.get(tailleListBases - 1)));
      }
  ...
  public void autoAnnotateHelices() {
    Stack<Integer> p = new Stack<Integer>();
    p.push(Integer.valueOf(0));
    int nbH = 1;
    while (!p.empty()) {
      int i = p.pop().intValue();
      if (i < _listeBases.size()) {
        ModeleBase mb = _listeBases.get(i);
        int j = mb.getElementStructure();
        if (j == -1) {
          p.push(Integer.valueOf(i + 1));
        } else {
          if (j > i) {
            ModeleBase mbp = _listeBases.get(j);
            p.push(Integer.valueOf(j + 1));
          ...
          if (mb.getElementStructure() != -1) {
            if (mb.getElementStructure() == i - 1) {
              try {
                ...
                addAnnotation(new TextAnnotation("T" + nbT++, t,
                    TextAnnotation.AnchorType.LOOP));
              } catch (Exception e) {
                e.printStackTrace();
              }
            }
            p.push(Integer.valueOf(i + k - 1));
          }
    ...
    while (!p.empty()) {
      ...
      if (i < _listeBases.size()) {
        ...
        if (j == -1) {
          ...
          while ((k < getSize()) && ((mb.getElementStructure() >= i)
          ...
          if (mb.getElementStructure() != -1) {
            if ((mb.getElementStructure() == i - 1) && !terminal) {
              try {
                ...
                addAnnotation(new TextAnnotation("I" + nbT++, t,
                    TextAnnotation.AnchorType.LOOP));
              } catch (Exception e) {
                e.printStackTrace();
              }
              p.push(Integer.valueOf(k - 1));
            }
          }
        } else {
          if (j > i) {
    ...
    try {
      ...
      while (type != StreamTokenizer.TT_EOF) {
        ...
        case (StreamTokenizer.TT_NUMBER):
          accumulator.add(Double.valueOf(st.nval));
          break;
        ...
        case (StreamTokenizer.TT_WORD):
          if (st.sval.equals("/sequence")) {
            ...
            double val = accumulator.get(accumulator.size() - 1).doubleValue();
            //System.err.println((char) type);      
            BP.put(newCoupleI(Math.min(i, j), Math.max(i, j)),
                Double.valueOf(val * val));
            accumulator.clear();
          } else if (inSeq) {
            sequence += st.sval;
          }
      ...
      for (int i = 0; i < getSize(); i++) {
        ...
        if (j != -1) {
          Couple<Integer, Integer> coor = newCoupleI(Math.min(i, j),
              Math.max(i, j));
          ...
          for (int k = 0; k < getSize(); k++) {
            Couple<Integer, Integer> coor = newCoupleI(Math.min(i, k),
                Math.max(i, k));
            if (BP.containsKey(coor)) {
              acc -= BP.get(coor).doubleValue();
            }
          }
          result.add(Double.valueOf(acc));
        }
      }
    } catch (IOException e) {
    ...
    try {
      ...
      if (type == '%') {
        ...
        while (type != StreamTokenizer.TT_EOF) {
          switch (type) {
          case (StreamTokenizer.TT_NUMBER):
            curVals.add(Double.valueOf(st.nval));
            break;
          case (StreamTokenizer.TT_EOL):
            if (curVals.size() > 0) {
  ...
  public Double[] getColorMapValues() {
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      values[i] = Double.valueOf(_listeBases.get(i).getValue());
    }
    return values;
  }
    ...
    while ((i < s.size()) && (j < _listeBases.size())) {
      ...
      if (mb instanceof ModeleBaseNucleotide) {
        ...
        ((ModeleBasesComparison) mb).setBase2(
            ((s.get(i + 1).length() > 0) ? s.get(i + 1).charAt(0) : ' '));
        i += 2;
        j++;
      } else
        j++;
  ...
  public RNA clone() {
    try {
      ...
      throw new RuntimeException("cannot clone class ["
          + this.getClass().getName() + "] via serialization: " + e.toString());
    }
  }

  /**
   * Returns the base at index <code>index</code>. Indices are contiguous in the
   * sequence over an interval <code>[0,this.getSize()-1]</code>, where
   * <code>n</code> is the length of the sequence.
   * 
   * @param index
   *        The index, <code>0 &le; index < this.getSize()</code>, of the base
   *        model
   ...
   * contiguous in the sequence, and belong to an interval <code>[0,n-1]</code>,
   * where <code>n</code> is the length of the sequence.
   * 
   * @param indices
   *        A Collection of indices <code>i</code>,
   *        <code>0 &le; index < this.getSize()</code>, where some base models
   *        are found.
  ...
  public void addHighlightRegion(int from, int to) {
    _listeRegionHighlights
        .add(new HighlightRegionAnnotation(getBasesBetween(from, to)));
  }
  ...
  public void globalRotation(double angleDegres) {
    if (_listeBases.size() > 0) {
      ...
      for (int i = 0; i < _listeBases.size(); i++) {
        ...
        x = Math.cos(angle) * (_listeBases.get(i).getCenter().getX() - centre.x)
            - Math.sin(angle)
                * (_listeBases.get(i).getCenter().getY() - centre.y)
            + centre.x;
        // y' = sin(theta)*(x-xc) + cos(theta)*(y-yc) + yc
        y = Math.sin(angle) * (_listeBases.get(i).getCenter().getX() - centre.x)
            + Math.cos(angle)
                * (_listeBases.get(i).getCenter().getY() - centre.y)
            + centre.y;
        ...
        // base
        // x' = cos(theta)*(x-xc) - sin(theta)*(y-yc) + xc
        x = Math.cos(angle) * (_listeBases.get(i).getCoords().getX() - centre.x)
            - Math.sin(angle)
                * (_listeBases.get(i).getCoords().getY() - centre.y)
            + centre.x;
        // y' = sin(theta)*(x-xc) + cos(theta)*(y-yc) + yc
        y = Math.sin(angle) * (_listeBases.get(i).getCoords().getX() - centre.x)
            + Math.cos(angle)
                * (_listeBases.get(i).getCoords().getY() - centre.y)
            + centre.y;
  ...
  public static boolean testDirectionality(Point2D.Double pi, Point2D.Double pj,
                                           Point2D.Double pk) {
  ...
  public boolean hasVirtualLoops() {
    ...
    return ((_drawMode != DRAW_MODE_LINEAR) && (_drawMode != DRAW_MODE_CIRCULAR)
        && (consecutiveBPs));
  }
  ...
  public String getHTMLDescription() {
    ...
    result += "<tr><td><b>Length:</b></td><td>" + this.getSize()
        + " nts</td></tr>";
    result += "<tr><td><b>Base-pairs:</b></td><td>" + this.getAllBPs().size()
        + " </td></tr>";
    return result + "</table>";
  }
  ...
  public RNA restrictTo(ArrayList<Integer> positions) {
    ...
    for (int i = 0; i < oldSeq.length(); i++) {
      matching[i] = j;
      if (!removedPos.contains(Integer.valueOf(i))) {
    ...
    for (ModeleBP m : getAllBPs()) {
      if (removedPos.contains(Integer.valueOf(m.getIndex5()))
          || removedPos.contains(Integer.valueOf(m.getIndex3()))) {
        int i5 = matching[m.getIndex5()];
        int i3 = matching[m.getIndex3()];
        ModeleBP msbp = new ModeleBP(result.getBaseAt(i5), result.getBaseAt(i3),
            m.getEdgePartner5(), m.getEdgePartner3(), m.getStericity());
