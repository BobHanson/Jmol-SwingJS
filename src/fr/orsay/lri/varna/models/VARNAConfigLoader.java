...
public class VARNAConfigLoader {
  ...
  public static String[][] getParameterInfo() {
    String[][] info = {
        ...
        { algoOpt, "String",
            "Drawing algorithm, choosen from [" + ALGORITHM_NAVIEW + ","
                + ALGORITHM_LINE + "," + ALGORITHM_RADIATE + ","
                + ALGORITHM_CIRCULAR + "]" }, //
        ...
        { bpStyleOpt, "String",
            "Look and feel for base pairs drawings, choosen from ["
                + LEONTIS_WESTHOF_BP_STYLE + "," + LEONTIS_WESTHOF_BP_STYLE_ALT
                + "," + NONE_BP_STYLE + "," + SIMPLE_BP_STYLE + ","
                + RNAVIZ_BP_STYLE + "]" }, //
        { chemProbOpt, "", "" }, //
        { colorMapOpt, "String",
  ...
  private VARNAPanel retrieveParametersValues() throws ExceptionParameterError {
    ...
    try {
      _numRows = Integer.parseInt(
          _parameterSource.getParameterValue(numRowsOpt, "" + _numRows));
    } catch (NumberFormatException e) {
    ...
    try {
      _numColumns = Integer.parseInt(
          _parameterSource.getParameterValue(numColumnsOpt, "" + _numColumns));
    } catch (NumberFormatException e) {
      throw new ExceptionParameterError(e.getMessage(),
    ...
    for (int i = 0; i < _numColumns; i++) {
      for (int j = 0; j < _numRows; j++) {
        try {
          ...
          try {
            _rotation = Double.parseDouble(_parameterSource.getParameterValue(
                rotationOpt + n, Double.toString(_rotation)));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(e.getMessage(),
                "'" + _parameterSource.getParameterValue(rotationOpt + n,
                    "" + _rotation)
                    + "' is not a valid float value for rotation!");
          }

          try {
            _colorMapMin = Double.parseDouble(
                _parameterSource.getParameterValue(colorMapMinOpt + n,
          ...
          try {
            _colorMapMax = Double.parseDouble(
                _parameterSource.getParameterValue(colorMapMaxOpt + n,
          ...
          try {
            _bpIncrement = Double.parseDouble(
                _parameterSource.getParameterValue(bpIncrementOpt + n,
          ...
          try {
            _periodResNum = Integer.parseInt(_parameterSource
                .getParameterValue(periodNumOpt + n, "" + _periodResNum));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(e.getMessage(), "'"
                + _parameterSource.getParameterValue(periodNumOpt + n,
                    "" + _periodResNum)
                + "' is not a valid integer value for the period of residue numbers!");
          }
          try {
            _titleSize = Integer.parseInt(_parameterSource
                .getParameterValue(titleSizeOpt + n, "" + _titleSize));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(e.getMessage(), "'"
                + _parameterSource.getParameterValue(titleSizeOpt + n,
                    "" + _titleSize)
                + "' is not a valid integer value for the number of rows !");
          }

          try {
            _zoom = Double.parseDouble(
                _parameterSource.getParameterValue(zoomOpt + n, "" + _zoom));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(e.getMessage(),
                "'" + _parameterSource.getParameterValue(zoomOpt + n,
                    "" + _zoom)
          ...
          try {
            _zoomAmount = Double.parseDouble(_parameterSource
                .getParameterValue(zoomAmountOpt + n, "" + _zoomAmount));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(e.getMessage(),
                "'" + _parameterSource.getParameterValue(zoomAmountOpt + n,
                    "" + _zoomAmount)
                    + "' is not a valid integer value for the zoom amount!");
          }

          try {
            _spaceBetweenBases = Double.parseDouble(
                _parameterSource.getParameterValue(spaceBetweenBasesOpt + n,
                    "" + _spaceBetweenBases));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(e.getMessage(),
                "'" + _parameterSource.getParameterValue(
                    spaceBetweenBasesOpt + n, "" + _spaceBetweenBases)
                    + "' is not a valid integer value for the base spacing!");
          }

          _drawBases = Boolean.parseBoolean(_parameterSource
              .getParameterValue(drawBasesOpt + n, "" + _drawBases));
          _fillBases = Boolean.parseBoolean(_parameterSource
              .getParameterValue(fillBasesOpt + n, "" + _fillBases));
          _autoHelices = Boolean.parseBoolean(_parameterSource
              .getParameterValue(autoHelicesOpt + n, "" + _autoHelices));
          _drawColorMap = Boolean.parseBoolean(_parameterSource
              .getParameterValue(drawColorMapOpt + n, "" + _drawColorMap));
          _drawBackbone = Boolean.parseBoolean(_parameterSource
              .getParameterValue(drawBackboneOpt + n, "" + _drawBackbone));
          _colorMapValues = _parameterSource.getParameterValue(colorMapOpt + n,
              _colorMapValues);
          _autoTerminalLoops = Boolean.parseBoolean(
              _parameterSource.getParameterValue(autoTerminalLoopsOpt + n,
                  "" + _autoTerminalLoops));
          _autoInteriorLoops = Boolean.parseBoolean(
              _parameterSource.getParameterValue(autoInteriorLoopsOpt + n,
                  "" + _autoInteriorLoops));
          _drawNC = Boolean.parseBoolean(
              _parameterSource.getParameterValue(drawNCOpt + n, "" + _drawNC));
          _flatExteriorLoop = Boolean.parseBoolean(
              _parameterSource.getParameterValue(flatExteriorLoopOpt + n,
                  "" + _flatExteriorLoop));
          _drawTertiary = Boolean.parseBoolean(_parameterSource
              .getParameterValue(drawTertiaryOpt + n, "" + _drawTertiary));
          _warning = Boolean.parseBoolean(
              _parameterSource.getParameterValue(warningOpt + n, "false"));
          _error = Boolean.parseBoolean(
              _parameterSource.getParameterValue(errorOpt + n, "true"));
          _border = parseDimension(
              _parameterSource.getParameterValue(borderOpt + n, "0X0"));
          ...
          _firstStructure = _parameterSource.getParameterValue(
              firstStructureForComparisonOpt + n, _firstStructure);
          _secondSequence = _parameterSource.getParameterValue(
          ...
          _annotations = _parameterSource.getParameterValue(annotationsOpt + n,
              _annotations);
          _URL = _parameterSource.getParameterValue(URLOpt + n, _URL);
          _algorithm = _parameterSource.getParameterValue(algoOpt + n,
              _algorithm);
          _customBases = _parameterSource.getParameterValue(customBasesOpt + n,
              _customBases);
          _auxBPs = _parameterSource.getParameterValue(auxBPsOpt + n, _auxBPs);
          _highlightRegion = _parameterSource
              .getParameterValue(highlightRegionOpt + n, _highlightRegion);
          _chemProbs = _parameterSource.getParameterValue(chemProbOpt + n,
              _chemProbs);
          _customBPs = _parameterSource.getParameterValue(customBPsOpt + n,
              _customBPs);
          _colorMapStyle = _parameterSource
              .getParameterValue(colorMapDefOpt + n, _colorMapStyle);
          _colorMapCaption = _parameterSource
              .getParameterValue(colorMapCaptionOpt + n, _colorMapCaption);
          _backboneColor = getSafeColor(_parameterSource.getParameterValue(
              backboneColorOpt + n, _backboneColor.toString()), _backboneColor);
          _backgroundColor = getSafeColor(
              _parameterSource.getParameterValue(backgroundColorOpt + n,
                  _backgroundColor.toString()),
              _backgroundColor);
          _bondColor = getSafeColor(_parameterSource.getParameterValue(
              bondColorOpt + n, _bondColor.toString()), _bondColor);
          _bpStyle = getSafeBPStyle(
              _parameterSource.getParameterValue(bpStyleOpt + n, ""), _bpStyle);
          _flip = _parameterSource.getParameterValue(flipOpt + n, _flip);
          _orientation = _parameterSource.getParameterValue(orientationOpt + n,
              _orientation);
          _titleColor = getSafeColor(_parameterSource.getParameterValue(
          ...
          if (_comparisonMode && _firstSequence != null
            ...
            _sseq = _parameterSource.getParameterValue(sequenceOpt + n, _sseq);
            _sstruct = _parameterSource.getParameterValue(structureOpt + n,
                _sstruct);
        ...
        } catch (ExceptionParameterError | ExceptionNonEqualLength | IOException
...
//      algoCode = RNA.DRAW_MODE_MOTIFVIEW;
  ...
  private void applyBasesStyle(String n) throws ExceptionParameterError {
    ...
    for (int numStyle = 0; numStyle < _basesStyleList.size(); numStyle++) {
      if (_basesStyleList.get(numStyle) != null) {
        tmp = _parameterSource.getParameterValue(
            applyBasesStyleOpt + (numStyle) + "on" + n, null);
          ...
          for (int k = 0; k < basesList.length; k++) {
            ...
            try {
              ...
              if (args.length == 1) {
                ...
                int index = _vp.getRNA().getIndexFromResidueNumber(baseNum);
                if (index != -1) {
                ...
                int indexTo = _vp.getRNA().getIndexFromResidueNumber(baseNumTo);
    ...
    for (int i = 0; i < baseStyles.length; i++) {
      ...
      try {
        if (data.length == 2) {
          ...
          if (index != -1) {
            String style = data[1];
            ModelBaseStyle msb = vp.getRNA().get_listeBases().get(index)
                .getStyleBase().clone();
      ...
      try {
        if (data.length == 2) {
          ...
          if (data2.length == 2) {
            ...
            ChemProbAnnotation cpa = new ChemProbAnnotation(mbl.get(c),
                mbl.get(d), style);
        ...
        if (data.length >= 1) {
          ...
          if (data2.length == 2) {
            ...
            if (s1.startsWith("(") && s2.endsWith(")")) {
              ...
              ModeleBP msbp = new ModeleBP(vp.getRNA().get_listeBases().get(c),
                  vp.getRNA().get_listeBases().get(d));
              if (data.length >= 2) {
                String style = data[1];
                msbp.assignParameters(style);
              }
  ...
  /**
   * Format: string:[type=[H|B|L|P]|x=double|y=double|anchor=int|size=int|color
   * =Color];
   * 
   * @param vp
   */
