...
public class VueUI {
   ...
   * BH SwingJS
   * 
   * A general method to handle all the showConfirmDialog callbacks from all the
   * VueXXX classes.
   * 
   * The initial return to be ignored is NaN, testable as value !=
   * Math.floor(value).
   * 
   * @param value
   * 
  ...
  public void openFileAsync() {
    if (_vp.isModifiable()) {
      ...
      Runnable ok = () -> {
        ...
        } catch (ExceptionLoadingFailed | ExceptionFileFormatOrSyntax
  ...
  public void rotateHelixAtom(int index, Point2D.Double newPos) {
    ...
    Point2D.Double limitLoopLeft, limitLoopRight, limitLeft, limitRight,
    ...
    Point2D.Double center = _vp.getRNA().get_listeBases().get(h.x).getCenter();
    double base = (RNA.computeAngle(center, limitLoopRight)
        + RNA.computeAngle(center, limitLoopLeft)) / 2.0;
  ...
  protected double normalizeAngle(double angle, double base) {
    while (angle < base) {
      angle += 2.0 * Math.PI;
    }
