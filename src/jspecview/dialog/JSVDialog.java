...
abstract public class JSVDialog extends Annotation implements AnnotationData {
  ...
  public void setFields() {
    ...
    case PeakList:
      ...
      dialog.setSelectedIndex(combo1, myParams.peakListInterpolation.equals("none") ? 1 : 0);
      createData();
      break;
    case OverlayLegend:
      break;
...
//  }
  ...
  private String getThreasholdText(double y) {
    if (Double.isNaN(y)) {
      ...
      y = (c == null
          ? (pd.getView().minYOnScale * f + pd.getView().maxYOnScale) * (1 - f)
          : c.getYVal());
    }
  ...
  protected void loadData() {
    ...
    case Measurements:
      if (xyData == null)
        return;
      data = xyData
          .getMeasurementListArray(dialog.getSelectedItem(combo1).toString());
      header = xyData.getDataHeader();
      widths = new int[] { 40, 65, 65, 50 };
      createTable(data, header, widths);
      break;
  ...
  private String getField(String url, String name) {
    ...
    return (pt < 0 ? null
        : url.substring(pt + key.length(), url.indexOf("&", pt + 1)));
  }
